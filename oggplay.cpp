...
PAL_FORCE_INLINE void OGG_FillResample(LPOGGPLAYER player, ogg_int16_t *stream) {
  if (gConfig.iAudioChannels == 2) {
    ...
    stream[1] = (player->vi.channels > 1) ? resampler_get_and_remove_sample(player->resampler[1])
                                          : stream[0];
  } else {
    if (player->vi.channels > 1) {
      *stream = (short)((int)(resampler_get_and_remove_sample(player->resampler[0]) +
                              resampler_get_and_remove_sample(player->resampler[1])) >>
                        1);
    } else {
...
static BOOL OGG_Rewind(LPOGGPLAYER player) {
  ...
  vorbis_info_init(&player->vi);
  player->iFlags |= FLAG_VI;
  vorbis_comment_init(&player->vc);
  player->iFlags |= FLAG_VC;
    ...
    while (i < 2) {
      ...
      if (result == 1) {
        ogg_stream_pagein(&player->os, &player->og); /* we can ignore any errors here
                                                                                                 as
                                                        they'll also become apparent at packetout */
        while (i < 2) {
  ...
  if (vorbis_synthesis_init(&player->vd, &player->vi) == 0) { /* central decode state */
    vorbis_block_init(&player->vd,
                      &player->vb); /* local state for most of the decode
                                                                                so multiple block
    ...
    if (player->fUseResampler) {
      ...
      for (i = 0; i < min(player->vi.channels, 2); i++) {
        resampler_set_quality(player->resampler[i], AUDIO_IsIntegerConversion(player->vi.rate)
...
static VOID OGG_FillBuffer(VOID *object, LPBYTE stream, INT len) {
  ...
  if (player->fReady) {
    ...
    while (total_bytes < len) {
      ...
      switch (stage) {
        ...
        case STAGE_PCMOUT:
          if ((samples = vorbis_synthesis_pcmout(&player->vd, &pcm)) > 0) {
            ...
            if (player->fUseResampler) { /* Resampler is available and should be used */
              bout = 0;
              while (total_bytes < len &&
                     samples > 0) { /* Fill as many samples into resampler as possible */
              ...
              for (i = 0; i < bout; i++) {
                if (gConfig.iAudioChannels == 2) {
                  ...
                  if (player->vi.channels > 1) {
                    ptr[0] =
                        (short)((int)(OGG_GetSample(pcm[0][i]) + OGG_GetSample(pcm[1][i])) >> 1);
                  } else {
                    ptr[0] = OGG_GetSample(pcm[0][i]);
                  }
                }
        ...
        case STAGE_REWIND:
          if (player->vi.rate !=
              gConfig.iSampleRate) { /* If there are samples in the resampler, fetch them first */
            while (total_bytes < len && resampler_get_sample_count(player->resampler[0]) > 0) {
              OGG_FillResample(player, (ogg_int16_t *)(stream + total_bytes));
              total_bytes += gConfig.iAudioChannels * sizeof(ogg_int16_t);
            }
...
static BOOL OGG_Play(VOID *object, INT iNum, BOOL fLoop, FLOAT flFadeTime) {
  LPOGGPLAYER player = (LPOGGPLAYER)object;
...
static VOID OGG_Shutdown(VOID *object) {
  if (object) {
    LPOGGPLAYER player = (LPOGGPLAYER)object;
    OGG_Cleanup(player);
