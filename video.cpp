...
static SDL_Texture *VIDEO_CreateTexture(int width, int height) {
  ...
  //
  return SDL_CreateTexture(gpRenderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING,
...
--*/
{
...
#if APPIMAGE
  SDL_Surface *surf = STBIMG_Load(PAL_va(0, "%s%s", dirname(dirname(dirname(gExecutablePath))),
                                         "/usr/share/icons/hicolor/256x256/apps/sdlpal.png"));
#endif
  ...
  if (gpWindow == NULL)
    gpWindow =
        SDL_CreateWindow("Pal", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                         gConfig.dwScreenWidth, gConfig.dwScreenHeight,
                         PAL_VIDEO_INIT_FLAGS | (gConfig.fFullScreen ? SDL_WINDOW_BORDERLESS : 0));
  ...
  gpRenderer =
      SDL_CreateRenderer(gpWindow, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
  ...
  gpScreenReal = SDL_CreateRGBSurface(SDL_SWSURFACE, 320, 200, 32, 0x00FF0000, 0x0000FF00,
  ...
  if (!gConfig.fEnableGLSL) SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, gConfig.pszScaleQuality);
  gpTexture = gRenderBackend.CreateTexture(render_w, render_h);
  if (gConfig.fEnableGLSL) SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "0");
...
#define PIXELS 1
  ...
  // and reuse it - that makes color seems overexposed
    ...
    SDL_Surface *temp = SDL_CreateRGBSurfaceFrom(pixels, PIXELS, PIXELS, 32, PIXELS, 0x00FF0000,
  ...
  //
  gpScreenReal =
      SDL_SetVideoMode(gConfig.dwScreenWidth, gConfig.dwScreenHeight, 8, PAL_VIDEO_INIT_FLAGS);
  ...
  if (gpScreenReal == NULL) {
    ...
    //
    gpScreenReal =
        SDL_SetVideoMode(640, 480, 8, SDL_SWSURFACE | (gConfig.fFullScreen ? SDL_FULLSCREEN : 0));
  }
...
--*/
{
  ...
  if (SDL_MUSTLOCK(gpScreenReal)) {
    if (SDL_LockSurface(gpScreenReal) < 0) return;
  }
...
--*/
{
  ...
  if (gpScreenReal == NULL) {
    ...
    //
    gpScreenReal =
        SDL_SetVideoMode(PAL_DEFAULT_WINDOW_WIDTH, PAL_DEFAULT_WINDOW_HEIGHT, bpp, SDL_SWSURFACE);
  }
...
VOID VIDEO_ToggleScaleScreen(VOID)
/*++
  Purpose:
...
--*/
{
  ...
  } else if (gConfig.dwScreenWidth == 640 && gConfig.dwScreenHeight == 480 &&
             !(flags & SDL_FULLSCREEN)) {
...
--*/
{
  ...
  if (gpScreenReal == NULL) {
    ...
    // Fall back to software windowed mode in default size.
    //
    gpScreenReal = SDL_SetVideoMode(PAL_DEFAULT_WINDOW_WIDTH, PAL_DEFAULT_WINDOW_HEIGHT,
                                    (bpp == 0) ? 8 : bpp, SDL_SWSURFACE);
  }
...
--*/
{
  ...
  SYSTEMTIME st;
  GetLocalTime(&st);
  sprintf(filename, "%04d%02d%02d%02d%02d%02d%03d.bmp", st.wYear, st.wMonth, st.wDay, st.wHour,
  ...
  sprintf(filename, "%04d%02d%02d%02d%02d%02d%03d.bmp", ptm->tm_year + 1900, ptm->tm_mon,
          ptm->tm_mday, ptm->tm_hour, ptm->tm_min, ptm->tm_sec, (int)(tv.tv_usec / 1000));
#endif
  ...
  for (i = 0; i < 12; i++) {
    for (j = 0; j < 6; j++) {
      ...
      if (g_wShakeTime != 0) {
        ...
        if (g_wShakeTime & 1) {
          dstrect.y =
              (screenRealY + screenRealHeight - g_wShakeLevel) * screenRealHeight / gpScreen->h;
        } else {
          dstrect.y = screenRealY;
        }
...
--*/
{
  ...
  SDL_Surface *dest = SDL_CreateRGBSurface(
      pSource->flags, pSize ? pSize->w : pSource->w, pSize ? pSize->h : pSource->h,
      pSource->format->BitsPerPixel, pSource->format->Rmask, pSource->format->Gmask,
