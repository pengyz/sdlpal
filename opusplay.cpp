...
static BOOL OPUS_Rewind(LPOPUSPLAYER player) {
  ...
  if (player->fUseResampler) {
    ...
    for (i = 0; i < 2; i++) {
      resampler_set_quality(player->resampler[i], AUDIO_IsIntegerConversion(48000)
...
static VOID OPUS_FillBuffer(VOID *object, LPBYTE stream, INT len) {
  int total_bytes;
  ...
  while (total_bytes < len) {
    ...
    if (!player->fRewind && player->iBufLen == 0) {
      ...
      if (read_count == 0) {
        if (player->fLoop) player->fRewind = TRUE;
...
static BOOL OPUS_Play(VOID *object, INT iNum, BOOL fLoop, FLOAT flFadeTime) {
  LPOPUSPLAYER player = (LPOPUSPLAYER)object;
  ...
  player->fp = op_open_file(
      UTIL_GetFullPathName(internal_buffer, PAL_GLOBAL_BUFFER_SIZE, gConfig.pszGamePath,
                           PAL_va(0, "opus%s%.2d.opus", PAL_NATIVE_PATH_SEPARATOR, iNum)),
      &ret);
  if (player->fp == NULL) {
    return FALSE;
  }
...
static VOID OPUS_Shutdown(VOID *object) {
  if (object) {
    LPOPUSPLAYER player = (LPOPUSPLAYER)object;
    OPUS_Cleanup(player);
