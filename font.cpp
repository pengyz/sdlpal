...
static void PAL_LoadEmbeddedFont(void) {
  ...
  //
  for (i = 0; i < nChars; i++) {
    wchar_t w = (wchar_buf[i] >= unicode_upper_base)
                    ? (wchar_buf[i] - unicode_upper_base + unicode_lower_top)
                    : wchar_buf[i];
...
INT PAL_LoadUserFont(LPCSTR pszBdfFileName)
/*++
  Purpose:
    ...
    if (state == 0) {
      if (strncmp(buf, "CHARSET_REGISTRY", 16) == 0) {
        ...
        // }
          ...
          TerminateOnError("Charset of %s is %s, which is not a supported yet.", pszBdfFileName,
                           buf);
        }
        ...
        sscanf(buf + bytes_consumed, "%d%n", &got_size, &bytes_now);
        bytes_consumed += bytes_now;
        sscanf(buf + bytes_consumed, "%d%n", &size_x, &bytes_now);
        bytes_consumed += bytes_now;
        sscanf(buf + bytes_consumed, "%d%n", &size_y, &bytes_now);
        bytes_consumed += bytes_now;
      ...
      /*else if (strncmp(buf, "SWIDTH", 6) == 0)
      {
        ...
        sscanf(buf + bytes_consumed, "%s%n", dwidth, &bytes_now);
        bytes_consumed += bytes_now;
        sscanf(buf + bytes_consumed, "%d%n", &dwidth_x, &bytes_now);
        bytes_consumed += bytes_now;
        sscanf(buf + bytes_consumed, "%d%n", &dwidth_y, &bytes_now);
        bytes_consumed += bytes_now;
        font_w = dwidth_x;
        if (font_w > 8 && font_w <= 16) font_w = 16;
        if (!cstate) {
          font_w_global = font_w;
        }
        ...
        int bytes_consumed = 0, bytes_now;
        char bbx[10];
        sscanf(buf + bytes_consumed, "%s%n", bbx, &bytes_now);
        bytes_consumed += bytes_now;
        sscanf(buf + bytes_consumed, "%d%n", &bbw, &bytes_now);
        bytes_consumed += bytes_now;
        sscanf(buf + bytes_consumed, "%d%n", &bbh, &bytes_now);
        bytes_consumed += bytes_now;
        sscanf(buf + bytes_consumed, "%d%n", &bbox, &bytes_now);
        bytes_consumed += bytes_now;
        sscanf(buf + bytes_consumed, "%d%n", &bboy, &bytes_now);
        bytes_consumed += bytes_now;
      ...
      if (strncmp(buf, "ENDCHAR", 7) == 0) {
        ...
        wchar_t wc[2] = {0};
        PAL_MultiByteToWideCharCP((CODEPAGE)codepage, (LPCSTR)szCp, 2, wc, 1);
        if (wc[0] != 0) {
          wchar_t w = (wc[0] >= unicode_upper_base)
                          ? (wc[0] - unicode_upper_base + unicode_lower_top)
                          : wc[0];
#if POINT_IMPORT
          if (font_w <= 8) {
            bFontGlyph[0] = bFontGlyph[14] = 0xff;
            for (int m = 1; m <= 14; m++) bFontGlyph[m] |= 0x81;
          } else {
            bFontGlyph[0] = bFontGlyph[1] = bFontGlyph[28] = bFontGlyph[29] = 0xff;
            for (int m = 2; m <= 27; m++) bFontGlyph[m] |= ((m % 2) ? 1 : 0x80);
          }
#endif
          if (w < sizeof(unicode_font) / sizeof(unicode_font[0])) {
            ...
            UTIL_LogOutput(LOGLEVEL_DEBUG,
                           "this user font has much code than unifont! ignoring %d\n", w);
          }
        }
...
#define PAL_LOAD_INTERNAL_FONT(fontdata, height)                                        \
  ...
  font_offset_x = (char *)calloc(1, fonts);
  font_offset_y = (char *)calloc(1, fonts);
  ...
  if (fUse8x8Font) {
    ...
    if (font_width[wChar] == 32) {
      for (i = 0; i < _font_height * 2 && dest < top; i += 2, dest += lpSurface->pitch) {
        ...
        for (j = 0; j < 8 && x + j + 8 + x_offset < lpSurface->w && x + j + 8 + x_offset >= 0;
             j++) {
...
int PAL_FontHeight(void) { return _font_height; }
