...
static AVIPlayState *PAL_ReadAVIInfo(FILE *fp, AVIPlayState *avi) {
  RIFFHeader hdr;
      ...
      case AVI_movi:
        ...
        if (current_level != 2 || !HAS_FLAG(flags, FLAGS_AVI_ALL_HEADERS)) {
          UTIL_LogOutput(LOGLEVEL_WARNING,
                         "RIFF 'movi' block does not appear at second level or the AVI does not "
                         "contain both video & audio!");
          return NULL;
        }
        //
        ...
        // Create surface
        //
        avi->surface = SDL_CreateRGBSurface(SDL_SWSURFACE, bih.biWidth, bih.biHeight,
                                            bih.biBitCount, 0x7C00, 0x03E0, 0x001F, 0x0000);
        //
        // Build SDL audio conversion info
        //
        SDL_BuildAudioCVT(&avi->cvt, (wfe.format.wBitsPerSample == 8) ? AUDIO_U8 : AUDIO_S16LSB,
                          wfe.format.nChannels, wfe.format.nSamplesPerSec, AUDIO_S16SYS,
                          AUDIO_GetDeviceSpec()->channels, AUDIO_GetDeviceSpec()->freq);
        //
        // Allocate chunk buffer
        // Since SDL converts audio in-place, we need to make the buffer large enough to hold
        // converted data
        //
        avi->dwBufferSize =
            aviHeader.dwSuggestedBufferSize * avi->cvt.len_mult + sizeof(RIFFChunkHeader);
        if (avi->dwBufferSize > 0)
          avi->pChunkBuffer = (uint8_t *)UTIL_malloc(avi->dwBufferSize);
        else
          avi->pChunkBuffer = NULL;
        //
        // Allocate audio buffer, the buffer size is large enough to hold two-second audio data
        //
        avi->dwAudBufLen = max(wfe.format.nAvgBytesPerSec * 2, aviHeader.dwSuggestedBufferSize) *
                           avi->cvt.len_mult;
        ...
        switch (block.type) {
          ...
          case AVI_strf:
            ...
            switch (streamHeader.fccType) {
              case AVI_vids:
                if (HAS_FLAG(flags, FLAGS_AVI_VIDEO_FORMAT) ||
                    (streamHeader.fccHandler != VIDS_MSVC &&
                     streamHeader.fccHandler != VIDS_msvc)) {
                  UTIL_LogOutput(LOGLEVEL_WARNING,
                                 "The AVI uses video codec with no built-in support, or video "
                                 "codec appeared before!");
                  return NULL;
                }
                ...
                if (bih.biBitCount != 16) {
                  UTIL_LogOutput(LOGLEVEL_WARNING,
                                 "Built-in AVI playing support only 16-bit video!");
                  return NULL;
                }
              ...
              case AVI_auds:
                if (HAS_FLAG(flags, FLAGS_AVI_AUDIO_FORMAT) || streamHeader.fccHandler != 0) {
                  UTIL_LogOutput(LOGLEVEL_WARNING,
                                 "The AVI uses audio codec with no built-in support, or audio "
                                 "codec appeared before!");
                  return NULL;
                }
...
static RIFFChunk *PAL_ReadDataChunk(FILE *fp, long endPos, void *userbuf, uint32_t buflen,
                                    int mult) {
  RIFFBlockHeader hdr;
    ...
    switch (hdr.type) {
      ...
      case AVI_00dc:
        ...
        if (userbuf && buflen >= sizeof(RIFFChunkHeader) + hdr.length)
          ...
          chunk = (RIFFChunk *)UTIL_malloc(sizeof(RIFFChunkHeader) +
                                           hdr.length * (hdr.type == AVI_01wb ? mult : 1));
...
static void PAL_AVIFeedAudio(AVIPlayState *avi, uint8_t *buffer, uint32_t size) {
  //
  ...
  while (size > 0) {
    uint32_t feed_size = (avi->dwAudioWritePos + size > avi->dwAudBufLen)
                             ? avi->dwAudBufLen - avi->dwAudioWritePos
                             : size;
...
void PAL_AVIInit(void) { gAVIPlayState.selfMutex = SDL_CreateMutex(); }

void PAL_AVIShutdown(void) { SDL_DestroyMutex(gAVIPlayState.selfMutex); }

static void PAL_RenderAVIFrameToSurface(SDL_Surface *lpSurface, const RIFFChunk *lpChunk) {
#define AV_RL16(x) ((((const uint8_t *)(x))[1] << 8) | ((const uint8_t *)(x))[0])
#define CHECK_STREAM_PTR(n)                        \
  if ((stream_ptr + n) > lpChunk->header.length) { \
    return;                                        \
  }
        ...
        if (colors[0] & 0x8000) {
          ...
          for (int pixel_y = 0; pixel_y < 4; pixel_y++) {
            for (int pixel_x = 0; pixel_x < 4; pixel_x++, flags >>= 1) {
              pixels[pixel_ptr++] =
                  colors[((pixel_y & 0x2) << 1) + (pixel_x & 0x2) + ((flags & 0x1) ^ 1)];
            }
            pixel_ptr -= row_dec;
          }
        } else {
...
VOID SDLCALL AVI_FillAudioBuffer(void *udata, uint8_t *stream, int len) {
  ...
  while (avi->fp != NULL && len > 0 && avi->dwAudioWritePos != avi->dwAudioReadPos) {
    uint32_t fill_size = (avi->dwAudioReadPos + len > avi->dwAudBufLen)
                             ? avi->dwAudBufLen - avi->dwAudioReadPos
                             : len;
...
void *AVI_GetPlayState(void) { return &gAVIPlayState; }
