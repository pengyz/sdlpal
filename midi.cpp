...
void MIDI_Play(int iNumRIX, BOOL fLoop) {
  ...
  if (gConfig.fIsWIN95) {
    g_pMidi = native_midi_loadsong(
        UTIL_GetFullPathName(PAL_BUFFER_SIZE_ARGS(0), gConfig.pszGamePath,
                             PAL_va(1, "Musics%s%.3d.mid", PAL_NATIVE_PATH_SEPARATOR, iNumRIX)));
  }
  ...
  if (!g_pMidi) {
    ...
    if ((fp = UTIL_OpenFile("midi.mkf")) != NULL) {
      if ((size = PAL_MKFGetChunkSize(iNumRIX, fp)) > 0 && (buf = (uint8_t *)UTIL_malloc(size))) {
