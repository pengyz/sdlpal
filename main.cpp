...
/**
 * @brief Initialize everything needed by the game.
 *
 * @return VOID
 */
static VOID PAL_Init(VOID) {
  ...
  VIDEO_SetWindowTitle(
      UTIL_va(UTIL_GlobalBuffer(0), PAL_GLOBAL_BUFFER_SIZE, "Pal %s%s%s%s",
...
--*/
{
  ...
  while (TRUE) {
    ...
    //
    for (i = 0; i < 9; i++) {
      LPCBITMAPRLE lpFrame = PAL_SpriteGetFrame(
          lpSpriteCrane, cranepos[i][2] = (cranepos[i][2] + (iCraneFrame & 1)) % 8);
...
--*/
{
  ...
  //
  if (UTIL_Platform_Init(argc, argv) != 0) return -1;
  ...
  //
  if (PAL_HAS_CONFIG_PAGE && gConfig.fLaunchSetting) return 0;

  //
  // If user requests a file-based log, then add it after the system-specific one.
  //
  if (gConfig.pszLogFile) UTIL_LogAddOutputCallback(UTIL_LogToFile, gConfig.iLogLevel);
