...
static int _default_input_event_filter(const SDL_Event *event, volatile PALINPUTSTATE *state) {
  return 0;
}
static void _default_input_shutdown_filter() {}

static void (*input_init_filter)() = _default_init_filter;
static int (*input_event_filter)(const SDL_Event *,
                                 volatile PALINPUTSTATE *) = _default_input_event_filter;
static void (*input_shutdown_filter)() = _default_input_shutdown_filter;

static const int g_KeyMap[][2] = {
    {SDLK_UP, kKeyUp},           {SDLK_KP_8, kKeyUp},       {SDLK_DOWN, kKeyDown},
    {SDLK_KP_2, kKeyDown},       {SDLK_LEFT, kKeyLeft},     {SDLK_KP_4, kKeyLeft},
    {SDLK_RIGHT, kKeyRight},     {SDLK_KP_6, kKeyRight},    {SDLK_ESCAPE, kKeyMenu},
    {SDLK_INSERT, kKeyMenu},     {SDLK_LALT, kKeyMenu},     {SDLK_RALT, kKeyMenu},
    {SDLK_KP_0, kKeyMenu},       {SDLK_RETURN, kKeySearch}, {SDLK_SPACE, kKeySearch},
    {SDLK_KP_ENTER, kKeySearch}, {SDLK_LCTRL, kKeySearch},  {SDLK_PAGEUP, kKeyPgUp},
    {SDLK_KP_9, kKeyPgUp},       {SDLK_PAGEDOWN, kKeyPgDn}, {SDLK_KP_3, kKeyPgDn},
    {SDLK_HOME, kKeyHome},       {SDLK_KP_7, kKeyHome},     {SDLK_END, kKeyEnd},
    {SDLK_KP_1, kKeyEnd},        {SDLK_r, kKeyRepeat},      {SDLK_a, kKeyAuto},
    {SDLK_d, kKeyDefend},        {SDLK_e, kKeyUseItem},     {SDLK_w, kKeyThrowItem},
    {SDLK_q, kKeyFlee},          {SDLK_f, kKeyForce},       {SDLK_s, kKeyStatus}};

static VOID PAL_KeyDown(INT key, BOOL fRepeat)
/*++
  Purpose:
...
static VOID PAL_UpdateKeyboardState(VOID)
/*++
  Purpose:
...
--*/
{
  ...
  if (lpEvent->type != SDL_MOUSEBUTTONDOWN && lpEvent->type != SDL_MOUSEBUTTONUP) return;
  ...
  switch (lpEvent->type) {
    ...
    case SDL_MOUSEBUTTONUP:
      ...
      hitTest++;
      if (abs(lastPressx - lastReleasex) < 25 && abs(lastPressy - lastReleasey) < 25) {
...
static VOID PAL_UpdateJoyStickState(VOID)
/*++
 Purpose:
 ...
 --*/
{
  if (g_InputState.axisX == 1 && g_InputState.axisY >= 0) {
    ...
    g_InputState.prevdir = g_InputState.dir;
    g_InputState.dir = kDirUnknown;
    if (!input_event_filter) g_InputState.dwKeyPress = kKeyNone;
  }
}
...
static int PAL_GetTouchArea(float X, float Y) {
...
static VOID PAL_SetTouchAction(int area) {
  switch (area) {
    case TOUCH_UP:
      g_InputState.dir = kDirNorth;
      g_InputState.dwKeyPress |= kKeyUp;
...
static VOID PAL_UnsetTouchAction(int area) {
  switch (area) {
    case TOUCH_UP:
    case TOUCH_DOWN:
    case TOUCH_LEFT:
...
static VOID PAL_TouchRepeatCheck(VOID) {
  if (!gConfig.fEnableKeyRepeat) {
    return;
  }
...
static int PAL_PollEvent(SDL_Event *event)
/*++
  Purpose:
...
#if PAL_HAS_JOYSTICKS
  if (g_InputState.joystickNeedUpdate) PAL_UpdateJoyStickState();
#endif
#if PAL_HAS_TOUCH
...
--*/
{
  if (init_filter) input_init_filter = init_filter;
  if (event_filter) input_event_filter = event_filter;
  if (shutdown_filter) input_shutdown_filter = shutdown_filter;
}
