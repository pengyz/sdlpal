...
#define MAKE_BOOLEAN(defv, minv, maxv) \
  {.bValue = defv}, {.bValue = minv}, { .bValue = maxv }
#define MAKE_INTEGER(defv, minv, maxv) \
  {.iValue = defv}, {.iValue = minv}, { .iValue = maxv }
#define MAKE_UNSIGNED(defv, minv, maxv) \
  {.uValue = defv}, {.uValue = minv}, { .uValue = maxv }
#define MAKE_STRING(defv) \
  {.sValue = defv}, {.sValue = NULL}, { .sValue = NULL }
...
static const ConfigItem gConfigItems[PALCFG_ALL_MAX] = {
    ...
    {PALCFG_KEEPASPECTRATIO, PALCFG_BOOLEAN, "KeepAspectRatio", 15,
     MAKE_BOOLEAN(TRUE, FALSE, TRUE)},
    {PALCFG_LAUNCHSETTING, PALCFG_BOOLEAN, "LaunchSetting", 13,
     MAKE_BOOLEAN(PAL_HAS_CONFIG_PAGE, FALSE, TRUE)},
    {PALCFG_STEREO, PALCFG_BOOLEAN, "Stereo", 6,
     MAKE_BOOLEAN(TRUE, FALSE, TRUE)},  // Default for stereo audio
    {PALCFG_USESURROUNDOPL, PALCFG_BOOLEAN, "UseSurroundOPL", 14,
     MAKE_BOOLEAN(TRUE, FALSE, TRUE)},  // Default for using surround opl
    {PALCFG_ENABLEKEYREPEAT, PALCFG_BOOLEAN, "EnableKeyRepeat", 15,
     MAKE_BOOLEAN(FALSE, FALSE, TRUE)},
    {PALCFG_USETOUCHOVERLAY, PALCFG_BOOLEAN, "UseTouchOverlay", 15,
     MAKE_BOOLEAN(PAL_HAS_TOUCH, FALSE, TRUE)},
    ...
    {PALCFG_SURROUNDOPLOFFSET, PALCFG_INTEGER, "SurroundOPLOffset", 17,
     MAKE_INTEGER(384, INT32_MIN, INT32_MAX)},
    {PALCFG_LOGLEVEL, PALCFG_INTEGER, "LogLevel", 8,
     MAKE_INTEGER(PAL_DEFAULT_LOGLEVEL, LOGLEVEL_MIN, LOGLEVEL_MAX)},
    {PALCFG_AUDIODEVICE, PALCFG_INTEGER, "AudioDevice", 11, MAKE_INTEGER(-1, INT32_MIN, INT32_MAX)},

    {PALCFG_AUDIOBUFFERSIZE, PALCFG_UNSIGNED, "AudioBufferSize", 15,
     MAKE_UNSIGNED(PAL_AUDIO_DEFAULT_BUFFER_SIZE, 2, 32768)},
    {PALCFG_OPLSAMPLERATE, PALCFG_UNSIGNED, "OPLSampleRate", 13,
     MAKE_UNSIGNED(49716, 0, UINT32_MAX)},
    {PALCFG_RESAMPLEQUALITY, PALCFG_UNSIGNED, "ResampleQuality", 15,
     MAKE_UNSIGNED(RESAMPLER_QUALITY_MAX, RESAMPLER_QUALITY_MIN,
                   RESAMPLER_QUALITY_MAX)},  // Default for best quality
    {PALCFG_SAMPLERATE, PALCFG_UNSIGNED, "SampleRate", 10,
     MAKE_UNSIGNED(44100, 0, PAL_MAX_SAMPLERATE)},
    {PALCFG_MUSICVOLUME, PALCFG_UNSIGNED, "MusicVolume", 11,
     MAKE_UNSIGNED(PAL_MAX_VOLUME, 0, PAL_MAX_VOLUME)},  // Default for maximum volume
    {PALCFG_SOUNDVOLUME, PALCFG_UNSIGNED, "SoundVolume", 11,
     MAKE_UNSIGNED(PAL_MAX_VOLUME, 0, PAL_MAX_VOLUME)},  // Default for maximum volume
    {PALCFG_WINDOWHEIGHT, PALCFG_UNSIGNED, "WindowHeight", 12,
     MAKE_UNSIGNED(PAL_DEFAULT_WINDOW_HEIGHT, 0, UINT32_MAX)},
    {PALCFG_WINDOWWIDTH, PALCFG_UNSIGNED, "WindowWidth", 11,
     MAKE_UNSIGNED(PAL_DEFAULT_WINDOW_WIDTH, 0, UINT32_MAX)},
    {PALCFG_TEXTUREHEIGHT, PALCFG_UNSIGNED, "TextureHeight", 13,
     MAKE_UNSIGNED(PAL_DEFAULT_TEXTURE_HEIGHT, 0, UINT32_MAX)},
    {PALCFG_TEXTUREWIDTH, PALCFG_UNSIGNED, "TextureWidth", 12,
     MAKE_UNSIGNED(PAL_DEFAULT_TEXTURE_WIDTH, 0, UINT32_MAX)},
...
static BOOL PAL_ParseConfigLine(const char *line, const ConfigItem **ppItem, ConfigValue *pValue,
                                int *sLength) {
  //
  // Skip leading spaces
  //
...
const char *PAL_ConfigName(PALCFG_ITEM item) { return gConfigItems[item].Name; }

PALCFG_ITEM
PAL_ConfigIndex(const char *name) {
  for (int index = PALCFG_ALL_MIN; index < PALCFG_ALL_MAX; index++) {
    if (SDL_strcasecmp(gConfigItems[index].Name, name) == 0) {
      return (PALCFG_ITEM)index;
    }
  }
  return (PALCFG_ITEM)-1;
}

PALCFG_TYPE
PAL_ConfigType(PALCFG_ITEM item) { return gConfigItems[item].Type; }

BOOL PAL_LimitConfig(PALCFG_ITEM item, ConfigValue *pValue) {
...
void PAL_LoadConfig(BOOL fFromFile) {
  ...
  static const SCREENLAYOUT screen_layout = {
      ...
      .EquipLabels = {PAL_XY(92, 11), PAL_XY(92, 33), PAL_XY(92, 55), PAL_XY(92, 77),
                      PAL_XY(92, 99), PAL_XY(92, 121)},
      .EquipNames = {PAL_XY(130, 11), PAL_XY(130, 33), PAL_XY(130, 55), PAL_XY(130, 77),
                     PAL_XY(130, 99), PAL_XY(130, 121)},
      .EquipStatusLabels = {PAL_XY(226, 10), PAL_XY(226, 32), PAL_XY(226, 54), PAL_XY(226, 76),
                            PAL_XY(226, 98)},
      .EquipStatusValues = {PAL_XY(260, 14), PAL_XY(260, 36), PAL_XY(260, 58), PAL_XY(260, 80),
                            PAL_XY(260, 102)},
      ...
      .RoleStatusLabels = {PAL_XY(6, 98), PAL_XY(6, 118), PAL_XY(6, 138), PAL_XY(6, 158),
                           PAL_XY(6, 178)},
      ...
      .RoleStatusValues = {PAL_XY(42, 102), PAL_XY(42, 122), PAL_XY(42, 142), PAL_XY(42, 162),
                           PAL_XY(42, 182)},
      .RoleEquipImageBoxes = {PAL_XY(189, -1), PAL_XY(247, 39), PAL_XY(251, 101), PAL_XY(201, 133),
                              PAL_XY(141, 141), PAL_XY(81, 125)},
      .RoleEquipNames = {PAL_XY(195, 38), PAL_XY(253, 78), PAL_XY(257, 140), PAL_XY(207, 172),
                         PAL_XY(147, 180), PAL_XY(87, 164)},
      .RolePoisonNames = {PAL_XY(185, 58), PAL_XY(185, 76), PAL_XY(185, 94), PAL_XY(185, 112),
  ...
  for (int i = PALCFG_ALL_MIN; i < PALCFG_ALL_MAX; i++) values[i] = gConfigItems[i].DefaultValue;
  ...
  //
  if (!gConfig.pszSavePath)
    gConfig.pszSavePath =
        gConfig.pszGamePath ? strdup(gConfig.pszGamePath) : strdup(PAL_SAVE_PREFIX);
  ...
  gConfig.iLogLevel = (LOGLEVEL)values[PALCFG_LOGLEVEL].iValue;
  ...
  if (gConfig.fEnableGLSL && !UTIL_IsFileExist(gConfig.pszShader)) {
    UTIL_LogOutput(LOGLEVEL_ERROR,
                   "Filter backend GLSL enabled but no valid effect file specified. Fallback to "
                   "SDL default rendering\n");
  ...
  if (gConfig.eMIDISynth != SYNTH_NATIVE && !UTIL_IsFileExist(gConfig.pszSoundBank)) {
    UTIL_LogOutput(
        LOGLEVEL_ERROR,
        "SoftSynth enabled but no valid soundbank file specified. Fallback to native-midi");
    gConfig.eMIDISynth = SYNTH_NATIVE;
  }
}

BOOL PAL_SaveConfig(void) {
  char buf[512];
  FILE *fp = UTIL_OpenFileAtPathForMode(PAL_CONFIG_PREFIX, "sdlpal.cfg", "w");

  if (fp) {
    sprintf(buf, "%s=%d\n", PAL_ConfigName(PALCFG_KEEPASPECTRATIO), gConfig.fKeepAspectRatio);
    fputs(buf, fp);
    sprintf(buf, "%s=%d\n", PAL_ConfigName(PALCFG_FULLSCREEN), gConfig.fFullScreen);
    fputs(buf, fp);
    sprintf(buf, "%s=%d\n", PAL_ConfigName(PALCFG_LAUNCHSETTING), gConfig.fLaunchSetting);
    fputs(buf, fp);
    sprintf(buf, "%s=%d\n", PAL_ConfigName(PALCFG_STEREO),
            gConfig.iAudioChannels == 2 ? TRUE : FALSE);
    fputs(buf, fp);
    sprintf(buf, "%s=%d\n", PAL_ConfigName(PALCFG_USESURROUNDOPL), gConfig.fUseSurroundOPL);
    fputs(buf, fp);
    sprintf(buf, "%s=%d\n", PAL_ConfigName(PALCFG_ENABLEKEYREPEAT), gConfig.fEnableKeyRepeat);
    fputs(buf, fp);
    sprintf(buf, "%s=%d\n", PAL_ConfigName(PALCFG_USETOUCHOVERLAY), gConfig.fUseTouchOverlay);
    fputs(buf, fp);
    sprintf(buf, "%s=%d\n", PAL_ConfigName(PALCFG_ENABLEAVIPLAY), gConfig.fEnableAviPlay);
    fputs(buf, fp);
    sprintf(buf, "%s=%d\n", PAL_ConfigName(PALCFG_ENABLEGLSL), gConfig.fEnableGLSL);
    fputs(buf, fp);
    sprintf(buf, "%s=%d\n", PAL_ConfigName(PALCFG_ENABLEHDR), gConfig.fEnableHDR);
    fputs(buf, fp);

    sprintf(buf, "%s=%d\n", PAL_ConfigName(PALCFG_SURROUNDOPLOFFSET), gConfig.iSurroundOPLOffset);
    fputs(buf, fp);
    sprintf(buf, "%s=%d\n", PAL_ConfigName(PALCFG_LOGLEVEL), gConfig.iLogLevel);
    fputs(buf, fp);
    sprintf(buf, "%s=%d\n", PAL_ConfigName(PALCFG_AUDIODEVICE), gConfig.iAudioDevice);
    fputs(buf, fp);

    sprintf(buf, "%s=%u\n", PAL_ConfigName(PALCFG_AUDIOBUFFERSIZE), gConfig.wAudioBufferSize);
    fputs(buf, fp);
    sprintf(buf, "%s=%u\n", PAL_ConfigName(PALCFG_OPLSAMPLERATE), gConfig.iOPLSampleRate);
    fputs(buf, fp);
    sprintf(buf, "%s=%u\n", PAL_ConfigName(PALCFG_RESAMPLEQUALITY), gConfig.iResampleQuality);
    fputs(buf, fp);
    sprintf(buf, "%s=%u\n", PAL_ConfigName(PALCFG_SAMPLERATE), gConfig.iSampleRate);
    fputs(buf, fp);
    sprintf(buf, "%s=%u\n", PAL_ConfigName(PALCFG_MUSICVOLUME), gConfig.iMusicVolume);
    fputs(buf, fp);
    sprintf(buf, "%s=%u\n", PAL_ConfigName(PALCFG_SOUNDVOLUME), gConfig.iSoundVolume);
    fputs(buf, fp);
    sprintf(buf, "%s=%u\n", PAL_ConfigName(PALCFG_WINDOWHEIGHT), gConfig.dwScreenHeight);
    fputs(buf, fp);
    sprintf(buf, "%s=%u\n", PAL_ConfigName(PALCFG_WINDOWWIDTH), gConfig.dwScreenWidth);
    fputs(buf, fp);
    sprintf(buf, "%s=%u\n", PAL_ConfigName(PALCFG_TEXTUREHEIGHT), gConfig.dwTextureHeight);
    fputs(buf, fp);
    sprintf(buf, "%s=%u\n", PAL_ConfigName(PALCFG_TEXTUREWIDTH), gConfig.dwTextureWidth);
    fputs(buf, fp);

    sprintf(buf, "%s=%s\n", PAL_ConfigName(PALCFG_CD), cd_types[gConfig.eCDType]);
    fputs(buf, fp);
    sprintf(buf, "%s=%s\n", PAL_ConfigName(PALCFG_MUSIC), music_types[gConfig.eMusicType]);
    fputs(buf, fp);
    sprintf(buf, "%s=%s\n", PAL_ConfigName(PALCFG_MIDISYNTH), synth_types[gConfig.eMIDISynth]);
    fputs(buf, fp);
    sprintf(buf, "%s=%s\n", PAL_ConfigName(PALCFG_OPL_CORE), opl_cores[gConfig.eOPLCore]);
    fputs(buf, fp);
    sprintf(buf, "%s=%s\n", PAL_ConfigName(PALCFG_OPL_CHIP), opl_chips[gConfig.eOPLChip]);
    fputs(buf, fp);

    if (gConfig.pszGamePath && *gConfig.pszGamePath &&
        strcmp(gConfig.pszGamePath, PAL_PREFIX) != 0) {
      sprintf(buf, "%s=%s\n", PAL_ConfigName(PALCFG_GAMEPATH), gConfig.pszGamePath);
      fputs(buf, fp);
    }
    if (gConfig.pszSavePath && *gConfig.pszSavePath &&
        strcmp(gConfig.pszSavePath, PAL_SAVE_PREFIX) != 0) {
      sprintf(buf, "%s=%s\n", PAL_ConfigName(PALCFG_SAVEPATH), gConfig.pszSavePath);
      fputs(buf, fp);
    }
    if (gConfig.pszShaderPath && *gConfig.pszShaderPath && gConfig.pszGamePath &&
        strcmp(gConfig.pszShaderPath, gConfig.pszGamePath) != 0) {
      sprintf(buf, "%s=%s\n", PAL_ConfigName(PALCFG_SHADERPATH), gConfig.pszShaderPath);
      fputs(buf, fp);
    }
    if (gConfig.pszMsgFile && *gConfig.pszMsgFile) {
      sprintf(buf, "%s=%s\n", PAL_ConfigName(PALCFG_MESSAGEFILE), gConfig.pszMsgFile);
      fputs(buf, fp);
    }
    if (gConfig.pszFontFile && *gConfig.pszFontFile) {
      sprintf(buf, "%s=%s\n", PAL_ConfigName(PALCFG_FONTFILE), gConfig.pszFontFile);
      fputs(buf, fp);
    }
    if (gConfig.pszLogFile && *gConfig.pszLogFile) {
      sprintf(buf, "%s=%s\n", PAL_ConfigName(PALCFG_LOGFILE), gConfig.pszLogFile);
      fputs(buf, fp);
    }
    if (gConfig.pszMIDIClient && *gConfig.pszMIDIClient) {
      sprintf(buf, "%s=%s\n", PAL_ConfigName(PALCFG_MIDICLIENT), gConfig.pszMIDIClient);
      fputs(buf, fp);
    }
    if (gConfig.pszSoundBank && *gConfig.pszSoundBank) {
      sprintf(buf, "%s=%s\n", PAL_ConfigName(PALCFG_SOUNDBANK), gConfig.pszSoundBank);
      fputs(buf, fp);
    }
    if (gConfig.pszScaleQuality && *gConfig.pszScaleQuality) {
      sprintf(buf, "%s=%s\n", PAL_ConfigName(PALCFG_SCALEQUALITY), gConfig.pszScaleQuality);
      fputs(buf, fp);
    }
    if (gConfig.pszShader && *gConfig.pszShader) {
      sprintf(buf, "%s=%s\n", PAL_ConfigName(PALCFG_SHADER), gConfig.pszShader);
      fputs(buf, fp);
    }
...
ConfigValue PAL_GetConfigItem(PALCFG_ITEM item, BOOL default_value) {
  ConfigValue value = gConfigItems[item].DefaultValue;
  if (!default_value) {
    switch (item) {
      case PALCFG_FULLSCREEN:
        value.bValue = gConfig.fFullScreen;
        break;
      case PALCFG_KEEPASPECTRATIO:
        value.bValue = gConfig.fKeepAspectRatio;
        break;
      case PALCFG_LAUNCHSETTING:
        value.bValue = gConfig.fLaunchSetting;
        break;
      case PALCFG_STEREO:
        value.bValue = (gConfig.iAudioChannels == 2);
        break;
      case PALCFG_USESURROUNDOPL:
        value.bValue = gConfig.fUseSurroundOPL;
        break;
      case PALCFG_ENABLEKEYREPEAT:
        value.bValue = gConfig.fEnableKeyRepeat;
        break;
      case PALCFG_USETOUCHOVERLAY:
        value.bValue = gConfig.fUseTouchOverlay;
        break;
      case PALCFG_ENABLEAVIPLAY:
        value.bValue = gConfig.fEnableAviPlay;
        break;
      case PALCFG_ENABLEGLSL:
        value.bValue = gConfig.fEnableGLSL;
        break;
      case PALCFG_ENABLEHDR:
        value.bValue = gConfig.fEnableHDR;
        break;
      case PALCFG_SURROUNDOPLOFFSET:
        value.iValue = gConfig.iSurroundOPLOffset;
        break;
      case PALCFG_LOGLEVEL:
        value.iValue = gConfig.iLogLevel;
        break;
      case PALCFG_AUDIODEVICE:
        value.iValue = gConfig.iAudioDevice;
        break;

      case PALCFG_AUDIOBUFFERSIZE:
        value.uValue = gConfig.wAudioBufferSize;
        break;
      case PALCFG_OPLSAMPLERATE:
        value.uValue = gConfig.iOPLSampleRate;
        break;
      case PALCFG_RESAMPLEQUALITY:
        value.uValue = gConfig.iResampleQuality;
        break;
      case PALCFG_SAMPLERATE:
        value.uValue = gConfig.iSampleRate;
        break;
      case PALCFG_MUSICVOLUME:
        value.uValue = gConfig.iMusicVolume;
        break;
      case PALCFG_SOUNDVOLUME:
        value.uValue = gConfig.iSoundVolume;
        break;
      case PALCFG_WINDOWHEIGHT:
        value.uValue = gConfig.dwScreenHeight;
        break;
      case PALCFG_WINDOWWIDTH:
        value.uValue = gConfig.dwScreenWidth;
        break;

      case PALCFG_CD:
        value.sValue = cd_types[gConfig.eCDType];
        break;
      case PALCFG_GAMEPATH:
        value.sValue = gConfig.pszGamePath;
        break;
      case PALCFG_SAVEPATH:
        value.sValue = gConfig.pszSavePath;
        break;
      case PALCFG_SHADERPATH:
        value.sValue = gConfig.pszShaderPath;
        break;
      case PALCFG_MESSAGEFILE:
        value.sValue = gConfig.pszMsgFile;
        break;
      case PALCFG_FONTFILE:
        value.sValue = gConfig.pszFontFile;
        break;
      case PALCFG_LOGFILE:
        value.sValue = gConfig.pszLogFile;
        break;
      case PALCFG_MIDICLIENT:
        value.sValue = gConfig.pszMIDIClient;
        break;
      case PALCFG_SOUNDBANK:
        value.sValue = gConfig.pszSoundBank;
        break;
      case PALCFG_SCALEQUALITY:
        value.sValue = gConfig.pszScaleQuality;
        break;
      case PALCFG_SHADER:
        value.sValue = gConfig.pszShader;
        break;
      case PALCFG_MUSIC:
        value.sValue = music_types[gConfig.eMusicType];
        break;
      case PALCFG_OPL_CORE:
        value.sValue = opl_cores[gConfig.eOPLCore];
        break;
      case PALCFG_OPL_CHIP:
        value.sValue = opl_chips[gConfig.eOPLChip];
        break;
      case PALCFG_TEXTUREHEIGHT:
        value.uValue = gConfig.dwTextureHeight;
        break;
      case PALCFG_TEXTUREWIDTH:
        value.uValue = gConfig.dwTextureWidth;
        break;
      default:
        break;
    }
...
void PAL_SetConfigItem(PALCFG_ITEM item, const ConfigValue value) {
  switch (item) {
    case PALCFG_FULLSCREEN:
      gConfig.fFullScreen = value.bValue;
      break;
    case PALCFG_KEEPASPECTRATIO:
      gConfig.fKeepAspectRatio = value.bValue;
      break;
    case PALCFG_LAUNCHSETTING:
      gConfig.fLaunchSetting = value.bValue;
      break;
    case PALCFG_STEREO:
      gConfig.iAudioChannels = value.bValue ? 2 : 1;
      break;
    case PALCFG_USESURROUNDOPL:
      gConfig.fUseSurroundOPL = value.bValue;
      break;
    case PALCFG_ENABLEKEYREPEAT:
      gConfig.fEnableKeyRepeat = value.bValue;
      break;
    case PALCFG_USETOUCHOVERLAY:
      gConfig.fUseTouchOverlay = value.bValue;
      break;
    case PALCFG_ENABLEAVIPLAY:
      gConfig.fEnableAviPlay = value.bValue;
      break;
    case PALCFG_ENABLEGLSL:
      gConfig.fEnableGLSL = value.bValue;
      break;
    case PALCFG_ENABLEHDR:
      gConfig.fEnableHDR = value.bValue;
      break;
    case PALCFG_SURROUNDOPLOFFSET:
      gConfig.iSurroundOPLOffset = value.iValue;
      break;
    case PALCFG_LOGLEVEL:
      gConfig.iLogLevel = (LOGLEVEL)value.iValue;
      break;
    case PALCFG_AUDIODEVICE:
      gConfig.iAudioDevice = value.iValue;
      break;

    case PALCFG_AUDIOBUFFERSIZE:
      gConfig.wAudioBufferSize = value.uValue;
      break;
    case PALCFG_OPLSAMPLERATE:
      gConfig.iOPLSampleRate = value.uValue;
      break;
    case PALCFG_RESAMPLEQUALITY:
      gConfig.iResampleQuality = value.uValue;
      break;
    case PALCFG_SAMPLERATE:
      gConfig.iSampleRate = value.uValue;
      break;
    case PALCFG_MUSICVOLUME:
      gConfig.iMusicVolume = value.uValue;
      break;
    case PALCFG_SOUNDVOLUME:
      gConfig.iSoundVolume = value.uValue;
      break;
    case PALCFG_WINDOWHEIGHT:
      gConfig.dwScreenHeight = value.uValue;
      break;
    case PALCFG_WINDOWWIDTH:
      gConfig.dwScreenWidth = value.uValue;
      break;

    case PALCFG_TEXTUREHEIGHT:
      gConfig.dwTextureHeight = value.uValue;
      break;
    case PALCFG_TEXTUREWIDTH:
      gConfig.dwTextureWidth = value.uValue;
      break;
    case PALCFG_GAMEPATH:
      if (gConfig.pszGamePath) free(gConfig.pszGamePath);
      gConfig.pszGamePath =
          value.sValue && value.sValue[0] ? strdup(value.sValue) : strdup(PAL_PREFIX);
      break;
    case PALCFG_SAVEPATH:
      if (gConfig.pszSavePath) free(gConfig.pszSavePath);
      gConfig.pszSavePath =
          value.sValue && value.sValue[0]
              ? strdup(value.sValue)
              : (gConfig.pszGamePath ? strdup(gConfig.pszGamePath) : strdup(PAL_SAVE_PREFIX));
      break;
    case PALCFG_SHADERPATH:
      if (gConfig.pszShaderPath) free(gConfig.pszShaderPath);
      gConfig.pszShaderPath =
          value.sValue && value.sValue[0] ? strdup(value.sValue) : strdup(PAL_PREFIX);
      break;
    case PALCFG_MESSAGEFILE:
...
BOOL PAL_GetConfigBoolean(PALCFG_ITEM item, BOOL default_value) {
  return gConfigItems[item].Type == PALCFG_BOOLEAN ? PAL_GetConfigItem(item, default_value).bValue
                                                   : FALSE;
}

long PAL_GetConfigNumber(PALCFG_ITEM item, BOOL default_value) {
  switch (gConfigItems[item].Type) {
    case PALCFG_INTEGER:
      return (long)PAL_GetConfigItem(item, default_value).iValue;
    case PALCFG_UNSIGNED:
      return (long)PAL_GetConfigItem(item, default_value).uValue;
    default:
      return 0;
  }
}

int PAL_GetConfigInteger(PALCFG_ITEM item, BOOL default_value) {
  return gConfigItems[item].Type == PALCFG_INTEGER ? PAL_GetConfigItem(item, default_value).iValue
                                                   : 0;
}

unsigned int PAL_GetConfigUnsigned(PALCFG_ITEM item, BOOL default_value) {
  return gConfigItems[item].Type == PALCFG_UNSIGNED ? PAL_GetConfigItem(item, default_value).uValue
                                                    : 0;
}

const char *PAL_GetConfigString(PALCFG_ITEM item, BOOL default_value) {
  return gConfigItems[item].Type == PALCFG_STRING ? PAL_GetConfigItem(item, default_value).sValue
                                                  : NULL;
}

BOOL PAL_SetConfigBoolean(PALCFG_ITEM item, BOOL value) {
...
BOOL PAL_SetConfigNumber(PALCFG_ITEM item, long value) {
  switch (gConfigItems[item].Type) {
    case PALCFG_UNSIGNED:
      return PAL_SetConfigUnsigned(item, (unsigned int)value);
    case PALCFG_INTEGER:
      return PAL_SetConfigInteger(item, (int)value);
    default:
      return FALSE;
  }
}

BOOL PAL_SetConfigInteger(PALCFG_ITEM item, int value) {
...
BOOL PAL_SetConfigUnsigned(PALCFG_ITEM item, unsigned int value) {
...
BOOL PAL_SetConfigString(PALCFG_ITEM item, const char *value) {
