...
struct {
  ...
  const char* levels;
    ...
    {"SDLPAL Launcher",
     ...
     "&Launch game",
     "&Default",
     "Verbose|Debug|Informational|Warning|Error|Fatal"},
    {"SDLPAL 启动器",
     "字体及语言设置",
     "显示设置",
     "音频设置",
...
void InitControls() {
  ...
  sprintf(buffer, "%u", gConfig.dwTextureWidth);
  gWidgets.texturewidth->value(buffer);
  sprintf(buffer, "%u", gConfig.dwTextureHeight);
  gWidgets.textureheight->value(buffer);
  sprintf(buffer, "%u", gConfig.dwScreenWidth);
  gWidgets.windowwidth->value(buffer);
  sprintf(buffer, "%u", gConfig.dwScreenHeight);
  gWidgets.windowheight->value(buffer);
  ...
  sprintf(buffer, "%d", gConfig.iSampleRate);
  gWidgets.samplerate->value(buffer);
  gWidgets.oplcore->value(gConfig.eOPLCore);
  gWidgets.oplchip->value(gConfig.eOPLChip);
  sprintf(buffer, "%d", gConfig.iOPLSampleRate);
  gWidgets.oplrate->value(buffer);
  gWidgets.surround->value(gConfig.fUseSurroundOPL ? 1 : 0);
  sprintf(buffer, "%d", gConfig.wAudioBufferSize);
  gWidgets.buffer->value(buffer);
...
void SaveControls() {
  ...
  gConfig.pszShader =
      *gWidgets.shaderfile->value() ? strdup(gWidgets.shaderfile->value()) : nullptr;
  ...
  gConfig.eOPLChip =
      gConfig.eOPLCore == OPLCORE_NUKED ? OPLCHIP_OPL3 : (OPLCHIP_TYPE)gWidgets.oplchip->value();
...
int GetLanguage() {
  ...
  if (strncasecmp(lang, "zh_", 3) == 0) {
    if (strncasecmp(lang + 3, "hans", 4) == 0 || strncasecmp(lang + 3, "CN", 2) == 0 ||
        strncasecmp(lang + 3, "SG", 2) == 0)
      return 1;
    else
      return 2;
  } else
...
Fl_Window* InitWindow() {
  ...
  (gWidgets.loglevel =
       new Fl_Choice(lang ? 115 : 85, 132, lang ? 90 : 120, 20, gLabels[lang].loglevel))
      ->add(gLabels[lang].levels);
  ...
  (gWidgets.oplcore = new Fl_Choice(224, 289, 75, 22, gLabels[lang].oplcore))
      ->add("MAME|DBFLT|DBINT|NUKED");
  ...
  (new Fl_Button(5, 420, 120, 24, gLabels[lang].exit))
      ->callback(
          [](Fl_Widget* ctrl, void* window) {
            if (ctrl->when() == FL_WHEN_RELEASE) static_cast<Fl_Window*>(window)->hide();
          },
          window);

  (new Fl_Button(260, 420, 120, 24, gLabels[lang].launch))
      ->callback(
          [](Fl_Widget* ctrl, void* window) {
  ...
  (new Fl_Button(515, 420, 120, 24, gLabels[lang].def))->callback([](Fl_Widget*) {
...
BOOL UTIL_IsAbsolutePath(LPCSTR lpszFileName) { return lpszFileName && *lpszFileName == '/'; }

INT UTIL_Platform_Init(int argc, char* argv[]) {
  openlog("sdlpal", LOG_PERROR | LOG_PID, LOG_USER);
  UTIL_LogAddOutputCallback(
      [](LOGLEVEL level, const char* str, const char*) -> void {
        const static int priorities[] = {LOG_DEBUG,   LOG_DEBUG, LOG_INFO,
                                         LOG_WARNING, LOG_ERR,   LOG_EMERG};
