...
static unsigned int yj1_get_bits(const void *src, unsigned int *bitptr, unsigned int count) {
  ...
  *bitptr += count;
  if (count > 16 - bptr) {
    count = count + bptr - 16;
    mask = 0xffff >> bptr;
    return (((temp[0] | (temp[1] << 8)) & mask) << count) |
           ((temp[2] | (temp[3] << 8)) >> (16 - count));
  } else
    return (((unsigned short)((temp[0] | (temp[1] << 8)) << bptr)) >> (16 - count));
}

static unsigned short yj1_get_loop(const void *src, unsigned int *bitptr,
...
static unsigned short yj1_get_count(const void *src, unsigned int *bitptr,
                                    PYJ_1_BLOCKHEADER header) {
  unsigned short temp;
...
INT YJ1_Decompress(LPCVOID Source, LPVOID Destination, INT DestSize) {
  ...
  if (SDL_SwapLE32(hdr->Signature) != 0x315f4a59) return -1;
  if (SDL_SwapLE32(hdr->UncompressedLength) > (unsigned int)DestSize) return -1;
  ...
  for (i = 0; i < SDL_SwapLE16(hdr->BlockCount); i++) {
    ...
    for (;;) {
      unsigned short loop;
      if ((loop = yj1_get_loop(src, &bitptr, header)) == 0) break;
...
static unsigned char yj2_data2[0x10] = {0x08, 0x05, 0x06, 0x04, 0x07, 0x05, 0x06, 0x03,
...
static void yj2_adjust_tree(YJ2_Tree tree, unsigned short value) {
  ...
  while (node->value != 0x280) {
    temp = node + 1;
    while (node->weight == temp->weight) temp++;
    temp--;
    if (temp != node) {
      tmp1 = node->parent;
...
static int yj2_build_tree(YJ2_Tree *tree) {
  ...
  YJ2_TreeNode **list;
  YJ2_TreeNode *node;
  if ((tree->list = list = (YJ2_TreeNode **)malloc(sizeof(YJ2_TreeNode *) * 321)) == NULL) return 0;
  ...
  for (i = 0; i <= 0x140; i++) list[i] = node + i;
  for (i = 0; i <= 0x280; i++) {
    node[i].value = i;
    node[i].weight = 1;
  }
...
INT YJ2_Decompress(LPCVOID Source, LPVOID Destination, INT DestSize) {
  ...
  if (!yj2_build_tree(&tree)) return -1;

  Length = SDL_SwapLE32(*((unsigned int *)Source));
  if (Length > DestSize) return -1;
  dest = (unsigned char *)Destination;
  ...
  while (1) {
    ...
    if (tree.node[0x280].weight == 0x8000) {
      int i;
      for (i = 0; i < 0x141; i++)
        if (tree.list[i]->weight & 0x1) yj2_adjust_tree(tree, i);
      for (i = 0; i <= 0x280; i++) tree.node[i].weight >>= 1;
    }
    ...
    if (val > 0xff) {
      ...
      unsigned char *pre;
      for (i = 0, temp = 0; i < 8; i++, ptr++) temp |= (unsigned int)yj2_bt(src, ptr) << i;
      tmp = temp & 0xff;
      for (; i < yj2_data2[tmp & 0xf] + 6; i++, ptr++) temp |= (unsigned int)yj2_bt(src, ptr) << i;
      temp >>= yj2_data2[tmp & 0xf];
      pos = (temp & 0x3f) | ((unsigned int)yj2_data1[tmp] << 6);
      if (pos == 0xfff) break;
      pre = dest - pos - 1;
      for (i = 0; i < val - 0xfd; i++) *dest++ = *pre++;
      len += val - 0xfd;
    } else {
