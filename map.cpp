...
--*/
{
  ...
  //
  if (iMapNum >= PAL_MKFGetChunkCount(fpMapMKF) || iMapNum >= PAL_MKFGetChunkCount(fpGopMKF) ||
      iMapNum <= 0) {
    return NULL;
  }
...
VOID PAL_MapBlitToSurface(LPCPALMAP lpMap, SDL_Surface *lpSurface, const SDL_Rect *lpSrcRect,
                          BYTE ucLayer)
/*++
  Purpose:
