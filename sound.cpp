...
static const void *SOUND_LoadWAVEData(LPCBYTE lpData, DWORD dwLen, WAVESPEC *lpSpec)
/*++
  Purpose:
...
static const void *SOUND_LoadVOCData(LPCBYTE lpData, DWORD dwLen, WAVESPEC *lpSpec)
/*++
  Purpose:
...
--*/
{
  ...
  if (dwLen < sizeof(VOCHEADER) || memcmp(lpVOC->signature, "Creative Voice File\x1A", 0x14) ||
      SDL_SwapLE16(lpVOC->data_offset) >= dwLen) {
    return NULL;
  }
...
static int SOUND_ResampleMix_U8_Mono_Mono(void *resampler[2], const void *lpData,
                                          const WAVESPEC *lpSpec, void *lpBuffer, int iBufLen,
                                          const void **llpData)
/*++
  Purpose:
...
--*/
{
  ...
  while (total_bytes < channel_len && src_samples > 0) {
    ...
    for (j = 0; j < to_write; j++) resampler_write_sample(resampler[0], (*src++ ^ 0x80) << 8);
...
static int SOUND_ResampleMix_U8_Mono_Stereo(void *resampler[2], const void *lpData,
                                            const WAVESPEC *lpSpec, void *lpBuffer, int iBufLen,
                                            const void **llpData)
/*++
  Purpose:
...
static int SOUND_ResampleMix_U8_Stereo_Mono(void *resampler[2], const void *lpData,
                                            const WAVESPEC *lpSpec, void *lpBuffer, int iBufLen,
                                            const void **llpData)
/*++
  Purpose:
  ...
  while (total_bytes < channel_len && src_samples > 0) {
    ...
    while (total_bytes < channel_len && resampler_get_sample_count(resampler[0]) > 0) {
      int sample = (((resampler_get_sample(resampler[0]) >> 8) +
                     (resampler_get_sample(resampler[1]) >> 8)) >>
                    1) +
                   *dst;
      *dst++ = (sample <= 32767) ? ((sample >= -32768) ? sample : -32768) : 32767;
      total_bytes += sizeof(short);
...
static int SOUND_ResampleMix_U8_Stereo_Stereo(void *resampler[2], const void *lpData,
                                              const WAVESPEC *lpSpec, void *lpBuffer, int iBufLen,
                                              const void **llpData)
/*++
  Purpose:
...
static int SOUND_ResampleMix_S16_Mono_Mono(void *resampler[2], const void *lpData,
                                           const WAVESPEC *lpSpec, void *lpBuffer, int iBufLen,
                                           const void **llpData)
/*++
  Purpose:

        Resample 16-bit signed (little-endian) mono PCM data into 16-bit signed (native-endian) mono
PCM data.
...
--*/
{
  ...
  while (total_bytes < channel_len && src_samples > 0) {
    int j, to_write = resampler_get_free_count(resampler[0]);
    if (to_write > src_samples) to_write = src_samples;
    for (j = 0; j < to_write; j++) resampler_write_sample(resampler[0], SDL_SwapLE16(*src++));
    src_samples -= to_write;
    while (total_bytes < channel_len && resampler_get_sample_count(resampler[0]) > 0) {
      int sample = (resampler_get_sample(resampler[0]) >> 8) + *dst;
      *dst++ = (sample <= 32767) ? ((sample >= -32768) ? sample : -32768) : 32767;
...
static int SOUND_ResampleMix_S16_Mono_Stereo(void *resampler[2], const void *lpData,
                                             const WAVESPEC *lpSpec, void *lpBuffer, int iBufLen,
                                             const void **llpData)
/*++
  Purpose:

        Resample 16-bit signed (little-endian) mono PCM data into 16-bit signed (native-endian)
stereo PCM data.
...
static int SOUND_ResampleMix_S16_Stereo_Mono(void *resampler[2], const void *lpData,
                                             const WAVESPEC *lpSpec, void *lpBuffer, int iBufLen,
                                             const void **llpData)
/*++
  Purpose:

        Resample 16-bit signed (little-endian) stereo PCM data into 16-bit signed (native-endian)
mono PCM data.
...
static int SOUND_ResampleMix_S16_Stereo_Stereo(void *resampler[2], const void *lpData,
                                               const WAVESPEC *lpSpec, void *lpBuffer, int iBufLen,
                                               const void **llpData)
/*++
  Purpose:
...
static BOOL SOUND_Play(VOID *object, INT iSoundNum, BOOL fLoop, FLOAT flFadeTime)
/*++
  Purpose:
...
--*/
{
  ...
  if (player->lastSFX == iSoundNum) return FALSE;
  ...
  snddata = player->LoadSound((LPCBYTE)buf, len, &wavespec);
  if (snddata == NULL) {
    free(buf);
    return FALSE;
  }
  ...
  if (wavespec.channels == 1 && devspec->channels == 1)
    mixer = (wavespec.format == AUDIO_S16) ? SOUND_ResampleMix_S16_Mono_Mono
                                           : SOUND_ResampleMix_U8_Mono_Mono;
  else if (wavespec.channels == 1 && devspec->channels == 2)
    mixer = (wavespec.format == AUDIO_S16) ? SOUND_ResampleMix_S16_Mono_Stereo
                                           : SOUND_ResampleMix_U8_Mono_Stereo;
  else if (wavespec.channels == 2 && devspec->channels == 1)
    mixer = (wavespec.format == AUDIO_S16) ? SOUND_ResampleMix_S16_Stereo_Mono
                                           : SOUND_ResampleMix_U8_Stereo_Mono;
  else if (wavespec.channels == 2 && devspec->channels == 2)
    mixer = (wavespec.format == AUDIO_S16) ? SOUND_ResampleMix_S16_Stereo_Stereo
  ...
  while (cursnd->next && cursnd->base) cursnd = cursnd->next;
  if (cursnd->base) {
  ...
  for (i = 0; i < wavespec.channels; i++) {
    ...
    resampler_set_quality(cursnd->resampler[i], AUDIO_IsIntegerConversion(wavespec.freq)
                                                    ? RESAMPLER_QUALITY_MIN
...
static VOID SOUND_FillBuffer(VOID *object, LPBYTE stream, INT len)
/*++
  Purpose:
...
--*/
{
  ...
  if (player) {
    ...
    do {
      if (cursnd->base) {
        cursnd->ResampleMix(cursnd->resampler, cursnd->current, &cursnd->spec, stream, len,
                            &cursnd->current);
