...
static LPWSTR gc_rgszDefaultAdditionalWords[ATB_WORD_COUNT] = {NULL,      L"\xFF11", L"\xFF12",
                                                               L"\xFF13", L"\xFF14", L"\xFF15"};
#endif

#define SDLPAL_EXTRA_WORD_COUNT 1
static LPCWSTR gc_rgszSDLPalWords[CP_MAX][SDLPAL_EXTRA_WORD_COUNT] = {
...
PAL_FORCE_INLINE int PAL_ParseLine(char *line, char **value, int *length, int deltrail) {
  //
...
PAL_FORCE_INLINE char *PAL_ReadOneLine(char *temp, int limit, FILE *fp) {
...
static int PAL_ReadMessageFile(FILE *fp) {
  ...
  while (!feof(fp)) {
    ...
    if ((buffer = PAL_ReadOneLine(temp, MESSAGE_MAX_BUFFER_SIZE, fp)) != NULL) {
      switch (state) {
          ...
          if (*buffer && *buffer != '#') {
            if (strncmp(buffer, "[BEGIN MESSAGE]", 15) == 0 &&
              ...
              UTIL_LogOutput(LOGLEVEL_WARNING,
                             "PAL_ReadMessageFile(): encounter invalid line '%s'!\n", buffer);
            }
          }
          break;
        case ST_DIALOG:
          ...
          //
          if (strncmp(buffer, "[END MESSAGE]", 13) == 0 && sscanf(buffer + 13, "%d", &eid) == 1 &&
              eid >= sid) {
            // End dialog
            state = ST_OUTSIDE;
            item->indexEnd = eid;
        ...
        case ST_WORD:
          ...
          if (strncmp(buffer, "[END WORDS]", 11) == 0) {
            ...
            if (i > 0) {
              ...
              struct _word_list_entry *val =
                  (struct _word_list_entry *)UTIL_malloc(sizeof(struct _word_list_entry));
            ...
            if (*line && *line != '#') {
              ...
              if (p) {
                ...
                *p++ = '\0';
                ...
                if (!lpObjectDesc) {
                  lpObjectDesc = (LPOBJECTDESC)UTIL_calloc(1, sizeof(OBJECTDESC));
                  memset(lpObjectDesc, 0, sizeof(OBJECTDESC));
                  if (prevObjectDesc) prevObjectDesc->next = lpObjectDesc;
                }
                if (isFirst) gpGlobals->lpObjectDesc = lpObjectDesc;
                ...
                *val = '\0';
                ...
                if (sscanf(line, "%s", index) == 1) {
                  if (strncasecmp(index, "UseISOFont", 10) == 0) {
                    ...
                    const char *szFontFlavors[] = {"Unifont", "SimpChin", "TradChin", "Japanese",
                                                   NULL};
            ...
            if ((i == 1 || (i >= 6 && i <= 11)) && !g_rcCredits[i]) {
              ...
              if (i == 6 || i == 7) {
                if (PAL_PLATFORM && PAL_CREDIT && PAL_PORTYEAR) {
                  ...
                  const int valuelen[] = {sizeof(PAL_PLATFORM) - 1, sizeof(PAL_CREDIT) - 1,
                                          sizeof(PAL_PORTYEAR) - 1};
          ...
          if (strncmp(buffer, "[END LAYOUT]", 12) == 0) {
            ...
            if (i >= 1 && i <= (sizeof(SCREENLAYOUT) / sizeof(PAL_POS))) {
              if ((n = sscanf(v, "%d,%d,%d", &x, &y, &f)) >= 2 && x < 320 && y < 200) {
                ...
                if (n == 3)
                  gConfig.ScreenLayoutFlag[i - 1] = (decltype(*gConfig.ScreenLayoutFlag))f;
              }
            }
          }
          break;
        default:
          TerminateOnError(
              "PAL_ReadMessageFile(): Reached an unknown state. Something really wrong may have "
              "happened!");
          break;
      }
  ...
  if (msg_cnt > 0) {
    ...
    for (item = head; item;) {
      ...
      if (g_TextLib.lpIndexBuf[item->index]) {
        //
        // If a MESSAGE with this sid exists, we firstly determine whether a larger block of memory
        // is needed to store msgSpan data.
        //
        if ((item->indexEnd - item->index + 1) > g_TextLib.indexMaxCounter[item->index]) {
          int oldCount = g_TextLib.indexMaxCounter[item->index];
          g_TextLib.lpIndexBuf[item->index] =
              (int **)realloc(g_TextLib.lpIndexBuf[item->index],
                              sizeof(int *) * (item->indexEnd - item->index + 1));
          // Update the corrisponding data in indexMaxCounter.
          g_TextLib.indexMaxCounter[item->index] = item->indexEnd - item->index + 1;
          // Clear the new allocated blocks; avoid it was then freed as pointer without actual being
          // allocated.
          memset(&g_TextLib.lpIndexBuf[item->index][oldCount], 0,
                 sizeof(int **) * (g_TextLib.indexMaxCounter[item->index] - oldCount));
        }
      } else {
        // It is a new MESSAGE. Give it a block of memory to store msgSpan data.
        g_TextLib.lpIndexBuf[item->index] =
            (int **)UTIL_calloc((item->indexEnd - item->index + 1), sizeof(int *));
        // Update the corrisponding data in indexMaxCounter.
        g_TextLib.indexMaxCounter[item->index] = item->indexEnd - item->index + 1;
      }
      //
      // If a duplicate MESSAGE appears, free the memory used by the previous one to avoid memory
      // leak.
      //
      ...
      g_TextLib.lpIndexBuf[item->index][item->indexEnd - item->index] =
          (int *)UTIL_calloc((item->count + 1), sizeof(int));
...
--*/
{
  ...
  if (gConfig.pszMsgFile) {
    ...
    if (!PAL_ReadMessageFile(fp)) {
      ...
      for (i = 0; i < 12; i++) {
        if (!g_rcCredits[i]) g_rcCredits[i] = (wchar_t *)L"";
      }
    }
  } else {
    FILE *fpMsg, *fpWord;
    ...
    for (i = 0, wpos = 0; i < g_TextLib.nWords; i++) {
      ...
      l = PAL_MultiByteToWideChar((LPCSTR)temp + i * gConfig.dwWordLength, gConfig.dwWordLength,
                                  g_TextLib.lpWordBuf[i], wlen - wpos);
      if (l > 0 && g_TextLib.lpWordBuf[i][l - 1] == '1') g_TextLib.lpWordBuf[i][l - 1] = 0;
    ...
    for (i = 0, wlen = 0; i < g_TextLib.nMsgs; i++) {
      wlen += PAL_MultiByteToWideChar((LPCSTR)temp + SDL_SwapLE32(offsets[i]),
                                      SDL_SwapLE32(offsets[i + 1]) - SDL_SwapLE32(offsets[i]), NULL,
                                      0) +
              1;
    }
    ...
    for (i = 0, wpos = 0; i < g_TextLib.nMsgs; i++) {
      ...
      l = PAL_MultiByteToWideChar((LPCSTR)temp + SDL_SwapLE32(offsets[i]),
                                  SDL_SwapLE32(offsets[i + 1]) - SDL_SwapLE32(offsets[i]),
                                  g_TextLib.lpMsgBuf[i], wlen - wpos);
    ...
    memcpy(g_TextLib.lpWordBuf + SYSMENU_LABEL_LAUNCHSETTING, gc_rgszSDLPalWords[PAL_GetCodePage()],
           SDLPAL_EXTRA_WORD_COUNT * sizeof(LPCWSTR));

#ifndef PAL_CLASSIC
    memcpy(g_TextLib.lpWordBuf + SYSMENU_LABEL_BATTLEMODE,
           gc_rgszAdditionalWords[PAL_GetCodePage()], ATB_WORD_COUNT * sizeof(LPCWSTR));
#endif
...
--*/
{
  return (iNumWord >= g_TextLib.nWords || !g_TextLib.lpWordBuf[iNumWord])
             ? L""
             : g_TextLib.lpWordBuf[iNumWord];
}
...
--*/
{
  return (iNumMsg >= g_TextLib.nMsgs || !g_TextLib.lpMsgBuf[iNumMsg]) ? L""
                                                                      : g_TextLib.lpMsgBuf[iNumMsg];
}
...
--*/
{
  ...
  assert(iSpan >= 0);
  assert(iOrder >= 0);
  return (iIndex >= g_TextLib.nMsgs || iSpan >= g_TextLib.indexMaxCounter[iIndex] ||
          !g_TextLib.lpIndexBuf[iIndex] || !g_TextLib.lpIndexBuf[iIndex][iSpan])
             ? -1
             : g_TextLib.lpIndexBuf[iIndex][iSpan][iOrder];
}
...
PAL_UnescapeText(LPCWSTR lpszText) {
  ...
  if (wcsstr(lpszText, L"\\") == NULL) return (LPWSTR)lpszText;
...
--*/
{
  ...
  if (fUnescape) lpszText = PAL_UnescapeText(lpszText);
...
--*/
{
  ...
  g_TextLib.posDialogTitle =
      PAL_XY(PAL_X(g_TextLib.posDialogTitle) + xOff, PAL_Y(g_TextLib.posDialogTitle) + yOff);
  g_TextLib.posDialogText =
      PAL_XY(PAL_X(g_TextLib.posDialogText) + xOff, PAL_Y(g_TextLib.posDialogText) + yOff);
  ...
  while (lpszText != NULL && *lpszText != '\0') {
    switch (*lpszText) {
      ...
      case '~':
        ...
        if (!isDialog) UTIL_Delay(wcstol(lpszText + 1, NULL, 10) * 80 / 7);
      ...
      default:
        ...
        if (isDialog) {
          if (g_TextLib.bCurrentFontColor == FONT_COLOR_DEFAULT) color = 0;
        ...
        // Update the screen on each draw operation is time-consuming, so disable it if user want to
        // skip
        if (isNumber)
          PAL_DrawNumber(text[0] - '0', 1, PAL_XY(x, y + 4), kNumColorYellow, kNumAlignLeft);
        else
          PAL_DrawTextUnescape(text, PAL_XY(x, y), color, !isDialog,
                               !isDialog && !g_TextLib.fUserSkip, FALSE, FALSE);
...
--*/
{
  ...
  if (g_TextLib.bDialogPosition == kDialogCenterWindow) {
...
#endif
    {
      ...
      for (i = 0; i < w; i++) len += PAL_CharWidth(lpszText[i]) >> 3;
      //
    ...
    if (g_TextLib.nCurrentDialogLine == 0 && g_TextLib.bDialogPosition != kDialogCenter &&
...
CODEPAGE
PAL_GetCodePage(void) { return g_codepage; }

void PAL_SetCodePage(CODEPAGE uCodePage) { g_codepage = uCodePage; }
...
PAL_DetectCodePageForString(const char *text, size_t text_len, CODEPAGE default_cp,
  ...
  if (text && text_len > 0) {
    ...
    for (int i = CP_BIG5; i <= CP_GBK; i++) {
      int invalids, length = PAL_MultiByteToWideCharCP((CODEPAGE)i, text, text_len, NULL, 0);
      WCHAR *wbuf = (WCHAR *)malloc(length * sizeof(WCHAR));
      PAL_MultiByteToWideCharCP((CODEPAGE)i, text, text_len, wbuf, length);
      for (int j = invalids = 0; j < length; j++) {
        int score = 1;
        for (int k = 0; k < sizeof(valid_ranges) / sizeof(valid_ranges[0]); k++) {
          if (wbuf[j] >= valid_ranges[k][0] && wbuf[j] <= valid_ranges[k][1]) {
            score = 0;
            break;
          }
        }
...
--*/
{
  ...
  if (!wcs) {
    switch (cp) {
      ...
      //		if (state == 0)
      //		{
      //			if ((BYTE)mbs[i] <= 0x80 || (BYTE)mbs[i] >= 0xfd || ((BYTE)mbs[i] >=
      //0xa0 && (BYTE)mbs[i] <= 0xdf)) 				wlen++; 			else 				state = 1;
      //		}
      //		else
      //		{
      //			wlen++;
    ...
    switch (cp) {
      ...
      //				wcs[wlen++] = cptbl_jis[(BYTE)mbs[i - 1] -
      //0x81][(BYTE)mbs[i] - 0x40]; 			else 				wcs[wlen++] = cptbl_jis[(BYTE)mbs[i - 1] -
      //0xc1][(BYTE)mbs[i] - 0x40]; 			state = 0;
      //		}
      //	}
      //	break;
      case CP_GBK:
    ...
    switch (state) {
      ...
      case 2:  // [width]
        switch (*format) {
          ...
          case '8':
          case '9':
            if (width >= 0) width = width * 10 + (*format - L'0');
            format++;
            continue;
          case '*':
            if (width == 0) width_var = 1;
            format++;
            continue;
          case '.':
            format++;
      ...
      case 3:  // [.precision]
        switch (*format) {
          ...
          case '8':
          case '9':
            if (precision >= 0) precision = precision * 10 + (*format - L'0');
            format++;
            continue;
          case '*':
            if (precision == 0) precision_var = 1;
            format++;
            continue;
          default:
            state = 4;
      ...
      case 5:  // type
        if (*format == 'c' || *format == 's') {
          ...
          if (precision > (int)len) precision = len;
          ...
          if (buffer + precision > buffer_end) precision = buffer_end - buffer;
          ...
          if (fmt_len < (size_t)(format - fmt_start + 1))
            cur_fmt =
                (LPWSTR)realloc(cur_fmt, ((fmt_len = format - fmt_start + 1) + 1) * sizeof(WCHAR));
