...
static VOID PAL_PartyRideEventObject(WORD wEventObjectID, INT x, INT y, INT h, INT iSpeed) {
  ...
  while (xOffset != 0 || yOffset != 0) {
    ...
    // Move the viewport
    //
    gpGlobals->viewport = PAL_XY(PAL_X(gpGlobals->viewport) + dx, PAL_Y(gpGlobals->viewport) + dy);
...
VOID PAL_AdditionalCredits(VOID) {
  LPCWSTR rgszcps[][CP_MAX] = {
      ...
      {L"", L"", /*L""*/},
      {
          L"   本程式是自由軟體，按照 GNU General", L"   本程序是自由软件，按照 GNU General",
          // L"
          // \x3053\x306E\x30D7\x30ED\x30B0\x30E9\x30E0\x306F\x81EA\x7531\x30BD\x30D5\x30C8\x30A6\x30A7\x30A2\x3067\x3059\x3001"
      },
      {
          L"   Public License v3 或更高版本發佈", L"   Public License v3 或更高版本发布",
          // L" GNU General Public License v3 \x306E\x4E0B\x3067"
      },
      {L"", L"", /*L" \x914D\x5E03\x3055\x308C\x3066\x3044\x307E\x3059\x3002"*/},
      {
          L"                    ...按 Enter 結束", L"                    ...按 Enter 结束",
  ...
  for (i = 0; i < 12; i++) {
    ...
    PAL_swprintf(buffer, sizeof(buffer) / sizeof(WCHAR), rgszStrings[i],
                 gConfig.pszMsgFile ? g_rcCredits[i] : rgszcps[i][PAL_GetCodePage()]);
  ...
  switch (pScript->wOperation) {
    ...
    case 0x001B:
      ...
      if (pScript->rgwOperand[0]) {
        ...
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
          ...
          if (PAL_IncreaseHPMP(w, (SHORT)(pScript->rgwOperand[1]), 0)) g_fScriptSuccess = TRUE;
        }
      } else {
        //
    ...
    case 0x001D:
      ...
      if (pScript->rgwOperand[0]) {
        ...
        // Apply to one player. The wEventObjectID parameter should indicate the player role.
        //
        if (!PAL_IncreaseHPMP(wEventObjectID, (SHORT)(pScript->rgwOperand[1]),
    ...
    case 0x0028:
      ...
      if (pScript->rgwOperand[0]) {
        ...
        for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
          ...
          if (RandomLong(0, 9) >= gpGlobals->g.rgObject[w].enemy.wResistanceToSorcery) {
            ...
            if (j >= MAX_POISONS) {
              for (j = 0; j < MAX_POISONS; j++) {
                if (g_Battle.rgEnemy[i].rgPoisons[j].wPoisonID == 0) {
                  ...
                  g_Battle.rgEnemy[i].rgPoisons[j].wPoisonScript = PAL_RunTriggerScript(
          ...
          if (j >= MAX_POISONS) {
            for (j = 0; j < MAX_POISONS; j++) {
              if (g_Battle.rgEnemy[wEventObjectID].rgPoisons[j].wPoisonID == 0) {
                ...
                g_Battle.rgEnemy[wEventObjectID].rgPoisons[j].wPoisonScript = PAL_RunTriggerScript(
                    gpGlobals->g.rgObject[pScript->rgwOperand[1]].poison.wEnemyScript,
                    wEventObjectID);
                break;
              }
    ...
    case 0x0033:
      ...
      if (g_Battle.rgEnemy[wEventObjectID].e.wCollectValue != 0) {
        gpGlobals->wCollectValue += g_Battle.rgEnemy[wEventObjectID].e.wCollectValue;
      } else {
    ...
    case 0x0034:
      ...
      if (gpGlobals->wCollectValue > 0) {
        ...
        if (gpGlobals->g.rgObject[wObject].item.wBitmap != wPrevImageIndex) {
          if (PAL_MKFReadChunk(bufImage, 2048, gpGlobals->g.rgObject[wObject].item.wBitmap,
    ...
    case 0x0037:
      ...
      // Play RNG animation
      //
      PAL_RNGPlay(gpGlobals->iCurPlayingRNG, pScript->rgwOperand[0],
    ...
    case 0x0038:
      ...
      if (!gpGlobals->fInBattle &&
        ...
        PAL_RunTriggerScript(gpGlobals->g.rgScene[gpGlobals->wNumScene - 1].wScriptOnTeleport,
                             0xFFFF);
      } else {
        //
        // failed
    ...
    case 0x0043:
      ...
      AUDIO_PlayMusic(pScript->rgwOperand[0], pScript->rgwOperand[1] != 1,
                      (pScript->rgwOperand[1] == 3 && pScript->rgwOperand[0] != 9) ? 3.0f : 0.0f);
      break;
    ...
    case 0x0049:
      ...
      //
      if (pScript->rgwOperand[0] != 0) pCurrent->sState = pScript->rgwOperand[1];
      break;
    ...
    case 0x0066:
      ...
      w += gpGlobals->g.PlayerRoles
               .rgwAttackStrength[gpGlobals->rgParty[g_Battle.wMovingPlayerIndex].wPlayerRole] *
           RandomFloat(0, 4);
    ...
    case 0x006D:
      ...
      if (pScript->rgwOperand[0]) {
        if (pScript->rgwOperand[1]) {
          gpGlobals->g.rgScene[pScript->rgwOperand[0] - 1].wScriptOnEnter = pScript->rgwOperand[1];
        }
    ...
    case 0x006E:
      ...
      gpGlobals->viewport = PAL_XY(PAL_X(gpGlobals->viewport) + (SHORT)(pScript->rgwOperand[0]),
    ...
    case 0x0080:
      ...
      PAL_PaletteFade(gpGlobals->wNumPalette, gpGlobals->fNightPalette, !(pScript->rgwOperand[0]));
      break;
    ...
    case 0x0081:
      ...
      //
      {
        ...
        x += ((gpGlobals->wPartyDirection == kDirWest || gpGlobals->wPartyDirection == kDirSouth)
                  ? 16
                  : -16);
        y += ((gpGlobals->wPartyDirection == kDirWest || gpGlobals->wPartyDirection == kDirNorth)
                  ? 8
                  : -8);
        ...
        if (abs(x) + abs(y * 2) < pScript->rgwOperand[1] * 32 + 16 &&
            gpGlobals->g.lprgEventObject[pScript->rgwOperand[0] - 1].sState > 0) {
          if (pScript->rgwOperand[1] > 0) {
            //
    ...
    case 0x0084:
      ...
      x += ((gpGlobals->wPartyDirection == kDirWest || gpGlobals->wPartyDirection == kDirSouth)
                ...
                : 16);
      y +=
          ((gpGlobals->wPartyDirection == kDirWest || gpGlobals->wPartyDirection == kDirNorth) ? -8
                                                                                               : 8);
        ...
        if (gpGlobals->fInBattle) {
          for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
            if (g_Battle.rgEnemy[i].wObjectID == g_Battle.rgEnemy[wEventObjectID].wObjectID) {
              count++;
              if (i == wEventObjectID) self_pos = count;
            }
          }
        }
        if (self_pos > 1) wScriptEntry = pScript->rgwOperand[0] - 1;
      }
      break;
    ...
    case 0x0096:
      ...
      //
      if (!gConfig.fIsWIN95) PAL_EndingAnimation();
      break;
    ...
    case 0x0098:
      ...
      for (i = 0; i < 2; i++)
        if (pScript->rgwOperand[i] > 0) {
          ...
          gpGlobals->rgParty[gpGlobals->wMaxPartyMemberIndex + curFollower].wPlayerRole =
              pScript->rgwOperand[i];
    ...
    case 0x009C:
      ...
      for (i = 0; i < MAX_ENEMIES_IN_TEAM; i++) {
        if (w > 0 && g_Battle.rgEnemy[i].wObjectID == 0) {
          ...
          g_Battle.rgEnemy[i].wScriptOnTurnStart =
              g_Battle.rgEnemy[wEventObjectID].wScriptOnTurnStart;
          g_Battle.rgEnemy[i].wScriptOnBattleEnd =
              g_Battle.rgEnemy[wEventObjectID].wScriptOnBattleEnd;
      ...
      g_Battle.rgEnemy[wCurEventObjectID].e.wHealth =
          (g_Battle.rgEnemy[wEventObjectID].e.wHealth + y) / x;

      w = 0;
      for (i = 0; i < MAX_ENEMIES_IN_TEAM; i++)
        if (g_Battle.rgEnemy[i].wObjectID != 0) w = i;
      ...
      if (x < y || g_Battle.iHidingTime > 0 ||
        ...
        for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
          if (g_Battle.rgEnemy[i].wObjectID == 0) {
            ...
            g_Battle.rgEnemy[i].state = kFighterWait;
            g_Battle.rgEnemy[i].wScriptOnTurnStart =
                gpGlobals->g.rgObject[w].enemy.wScriptOnTurnStart;
            g_Battle.rgEnemy[i].wScriptOnBattleEnd =
                gpGlobals->g.rgObject[w].enemy.wScriptOnBattleEnd;
    ...
    case 0x00A0:
      ...
      // Quit game
      //
      if (gConfig.fIsWIN95) PAL_EndingScreen();
    ...
    default:
      TerminateOnError("SCRIPT: Invalid Instruction at %4x: (%4x - %4x, %4x, %4x)", wScriptEntry,
...
PAL_FORCE_INLINE INT MESSAGE_GetSpan(WORD *pwScriptEntry) {
  ...
  //
  if (gpGlobals->g.lprgScriptEntry[currentScriptEntry + 1].wOperation == 0xFFFF &&
      gpGlobals->g.lprgScriptEntry[currentScriptEntry + 1].rgwOperand[0] != lastMsgIndex + 1)
    currentScriptEntry++;
  else
    while ((gpGlobals->g.lprgScriptEntry[currentScriptEntry].wOperation == 0xFFFF &&
            (!beginning ? gpGlobals->g.lprgScriptEntry[currentScriptEntry].rgwOperand[0] ==
                              lastMsgIndex + 1
                        : 1)) ||
  ...
  while (wScriptEntry != 0 && !fEnded) {
    ...
    switch (pScript->wOperation) {
      ...
      case 0x003C:
        ...
        //
        PAL_ClearDialog(TRUE);
        PAL_StartDialog(kDialogUpper, (BYTE)pScript->rgwOperand[1], pScript->rgwOperand[0],
                        pScript->rgwOperand[2] ? TRUE : FALSE);
        wScriptEntry++;
        break;
      ...
      case 0x003D:
        ...
        //
        PAL_ClearDialog(TRUE);
        PAL_StartDialog(kDialogLower, (BYTE)pScript->rgwOperand[1], pScript->rgwOperand[0],
                        pScript->rgwOperand[2] ? TRUE : FALSE);
        wScriptEntry++;
        break;
...
begin:
  ...
  UTIL_LogOutput(LOGLEVEL_DEBUG, "[AUTOSCRIPT] %04x %.4x: %.4x %.4x %.4x %.4x\n", wEventObjectID,
                 wScriptEntry, pScript->wOperation, pScript->rgwOperand[0], pScript->rgwOperand[1],
                 pScript->rgwOperand[2]);
  ...
  switch (pScript->wOperation) {
    ...
    case 0xFFFF:
      if (gConfig.fIsWIN95) {
        ...
        int YBase = (wEventObjectID & PAL_ITEM_DESC_BOTTOM)
                        ? 151 - gConfig.ScreenLayout.ExtraItemDescLines * 16
                        : 3;
        ...
        if (gConfig.pszMsgFile) {
          ...
          while ((iMsg = PAL_GetMsgNum(pScript->rgwOperand[0], msgSpan, idx++)) >= 0) {
            if (iMsg > 0) {
              PAL_DrawText(PAL_GetMsg(iMsg), PAL_XY(XBase, iDescLine * 16 + YBase), DESCTEXT_COLOR,
                           TRUE, FALSE, FALSE);
              iDescLine++;
            }
          }
        } else {
          PAL_DrawText(PAL_GetMsg(pScript->rgwOperand[0]), PAL_XY(XBase, iDescLine * 16 + YBase),
                       DESCTEXT_COLOR, TRUE, FALSE, FALSE);
          wScriptEntry++;
        }
      } else {
        wScriptEntry++;
