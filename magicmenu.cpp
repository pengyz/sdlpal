...
--*/
{
  ...
  if (!gConfig.fIsWIN95) {
    if (gpGlobals->lpObjectDesc == NULL) {
      ...
      PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen,
                           PAL_XY(260, 14));
      PAL_DrawNumber(rgMagicItem[g_iCurrentItem].wMP, 4, PAL_XY(230, 14), kNumColorYellow,
                     kNumAlignRight);
      PAL_DrawNumber(g_wPlayerMP, 4, PAL_XY(265, 14), kNumColorCyan, kNumAlignRight);
    } else {
      WCHAR szDesc[512], *next;
      const WCHAR *d =
          PAL_GetObjectDesc(gpGlobals->lpObjectDesc, rgMagicItem[g_iCurrentItem].wMagic);
      ...
      PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen,
                           PAL_XY(45, 14));
      PAL_DrawNumber(rgMagicItem[g_iCurrentItem].wMP, 4, PAL_XY(15, 14), kNumColorYellow,
                     kNumAlignRight);
    ...
    for (k = 0; k < iItemsPerLine; k++) {
      ...
      if (i == g_iCurrentItem) {
        PAL_RLEBlitToSurface(
            PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_CURSOR), gpScreen,
            PAL_XY(35 + iCursorXOffset + k * iItemTextWidth, 64 + j * 18 + iBoxYOffset));
      }
  ...
  if (g_InputState.dwKeyPress & kKeySearch) {
    if (rgMagicItem[g_iCurrentItem].fEnabled) {
      ...
      k = (g_iCurrentItem < iItemsPerLine * iPageLineOffset) ? (g_iCurrentItem / iItemsPerLine)
                                                             : iPageLineOffset;
