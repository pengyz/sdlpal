...
static float sinc(float x) { return fEqual(x, 0.0f) ? 1.0f : (float)(sin(x * M_PI) / (x * M_PI)); }
...
static inline void __cpuid(int *data, int selector) {
  ...
  asm("xchg{q} {%%}rbx, %q1; cpuid; xchg{q} {%%}rbx, %q1"
      ...
      : "0"(selector));
#else
  asm("cpuid" : "=a"(data[0]), "=b"(data[1]), "=c"(data[2]), "=d"(data[3]) : "0"(selector));
#endif
}
#else
...
static int resampler_run_blep_sse(resampler *r, float **out_, float *out_end) {
  ...
  if (in_size > 0) {
    ...
    do {
      ...
      if (out + SINC_WIDTH * 2 > out_end) break;
...
static int resampler_run_sinc_sse(resampler *r, float **out_, float *out_end) {
  ...
  if (in_size > 0) {
    ...
    int step = phase_inc > RESAMPLER_RESOLUTION
                   ? RESAMPLER_RESOLUTION * RESAMPLER_RESOLUTION / phase_inc
    ...
    do {
      ...
      for (; i >= -SINC_WIDTH + 1; --i) {
        ...
        kernel_sum += kernelf[i + SINC_WIDTH - 1] =
            sinc_lut[abs(phase_adj - pos)] * window_lut[abs(phase - window_pos)];
      }
      for (i = 0; i < SINC_WIDTH / 2; ++i) {
...
static int resampler_run_blep_c(resampler *r, float **out_, float *out_end) {
  ...
  if (in_size > 0) {
    ...
    do {
      ...
      last_amp += sample;
      sample /= kernel_sum;
      for (i = 0; i < SINC_WIDTH * 2; ++i) out[i] += sample * kernel[i];
...
static int resampler_run_cubic_c(resampler *r, float **out_, float *out_end) {
  ...
  if (in_size > 0) {
    ...
    do {
      ...
      for (sample = 0, i = 0; i < 4; ++i) sample += in[i] * kernel[i];
      *out++ = sample;
...
static int resampler_run_sinc_c(resampler *r, float **out_, float *out_end) {
  ...
  if (in_size > 0) {
    ...
    do {
      ...
      for (sample = 0, i = 0; i < SINC_WIDTH * 2; ++i) sample += in[i] * kernel[i];
...
void resampler_clear(void *_r) {
  ...
  if (r->quality == RESAMPLER_QUALITY_BLEP) memset(r->buffer_out, 0, sizeof(r->buffer_out));
}
  ...
  while (r->write_filled > min_filled && r->read_filled < resampler_buffer_size) {
    ...
    switch (quality) {
      ...
      case RESAMPLER_QUALITY_BLEP: {
        int used;
        int write_extra = 0;
        if (write_pos >= r->read_pos) write_extra = r->read_pos;
        if (write_extra > SINC_WIDTH * 2 - 1) write_extra = SINC_WIDTH * 2 - 1;
...
static void resampler_fill_and_remove_delay(resampler *r) {
  ...
  if (r->delay_removed < 0) {
    ...
    r->delay_removed = 0;
    while (delay--) resampler_remove_sample(r);
  }
}
...
void resampler_remove_sample(void *_r) {
  ...
  if (r->read_filled > 0) {
    if (r->quality == RESAMPLER_QUALITY_BLEP) {
      ...
      if (fabs(r->accumulator) < 1e-20) r->accumulator = 0;
    }
    --r->read_filled;
