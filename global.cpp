...
BOOL PAL_IsWINVersion(BOOL *pfIsWIN95) {
  FILE *fps[] = {UTIL_OpenRequiredFile("abc.mkf"),
                 UTIL_OpenRequiredFile("map.mkf"),
                 gpGlobals->f.fpF,
                 gpGlobals->f.fpFBP,
                 gpGlobals->f.fpFIRE,
                 gpGlobals->f.fpMGO};
  ...
  if (word_buf) {
    ...
    if (probability == 100)
      ...
      UTIL_LogOutput(LOGLEVEL_WARNING,
                     "PAL_DetectCodePage detected the most possible (%d) code page '%s' for %s\n",
...
VOID PAL_FreeGlobals(VOID) {
  ...
  //
  if (!gConfig.fIsWIN95) PAL_FreeObjectDesc(gpGlobals->lpObjectDesc);
...
--*/
{
  ...
  LOAD_DATA(p->lprgLevelUpMagic, p->nLevelUpMagic * sizeof(LEVELUPMAGIC_ALL), 6,
            gpGlobals->f.fpDATA);
...
#define PAL_DOALLOCATE(fp, num, type, lptype, ptr, n)                         \
  ...
  if (gpGlobals->g.lprgEventObject == NULL) {
    PAL_DOALLOCATE(gpGlobals->f.fpSSS, 0, EVENTOBJECT, LPEVENTOBJECT, gpGlobals->g.lprgEventObject,
                   gpGlobals->g.nEventObject);

    PAL_DOALLOCATE(gpGlobals->f.fpSSS, 4, SCRIPTENTRY, LPSCRIPTENTRY, gpGlobals->g.lprgScriptEntry,
                   gpGlobals->g.nScriptEntry);

    PAL_DOALLOCATE(gpGlobals->f.fpDATA, 0, STORE, LPSTORE, gpGlobals->g.lprgStore,
                   gpGlobals->g.nStore);

    PAL_DOALLOCATE(gpGlobals->f.fpDATA, 1, ENEMY, LPENEMY, gpGlobals->g.lprgEnemy,
                   gpGlobals->g.nEnemy);

    PAL_DOALLOCATE(gpGlobals->f.fpDATA, 2, ENEMYTEAM, LPENEMYTEAM, gpGlobals->g.lprgEnemyTeam,
                   gpGlobals->g.nEnemyTeam);

    PAL_DOALLOCATE(gpGlobals->f.fpDATA, 4, MAGIC, LPMAGIC, gpGlobals->g.lprgMagic,
                   gpGlobals->g.nMagic);

    PAL_DOALLOCATE(gpGlobals->f.fpDATA, 5, BATTLEFIELD, LPBATTLEFIELD, gpGlobals->g.lprgBattleField,
                   gpGlobals->g.nBattleField);
...
static BOOL PAL_LoadGame_Common(int iSaveSlot, LPSAVEDGAME_COMMON s, size_t size) {
  //
...
static INT PAL_LoadGame_DOS(int iSaveSlot)
/*++
  Purpose:
...
--*/
{
  ...
  //
  if (!PAL_LoadGame_Common(iSaveSlot, (LPSAVEDGAME_COMMON)s, sizeof(SAVEDGAME_DOS))) return -1;
...
--*/
{
  ...
  // Get all the data from the saved game struct.
  //
  if (!PAL_LoadGame_Common(iSaveSlot, (LPSAVEDGAME_COMMON)s, sizeof(SAVEDGAME_WIN))) return -1;
...
static VOID PAL_SaveGame_Common(int iSaveSlot, WORD wSavedTimes, LPSAVEDGAME_COMMON s,
                                size_t size) {
  FILE *fp;
  size_t i;
  ...
  // Try writing to file
  //
  if ((fp = UTIL_OpenFileAtPathForMode(gConfig.pszSavePath, PAL_va(1, "%d.rpg", iSaveSlot),
                                       "wb")) == NULL) {
    return;
  }
...
static VOID PAL_SaveGame_DOS(int iSaveSlot, WORD wSavedTimes)
/*++
  Purpose:
...
static VOID PAL_SaveGame_WIN(int iSaveSlot, WORD wSavedTimes)
/*++
  Purpose:
  ...
  if (iNum > 0) {
    ...
    if (fFound) {
      ...
      if (gpGlobals->rgInventory[index].nAmount == 0 && index == gpGlobals->iCurInvMenuItem &&
          index + 1 < MAX_INVENTORY && gpGlobals->rgInventory[index + 1].nAmount <= 0)
...
BOOL PAL_IncreaseHPMP(WORD wPlayerRole, SHORT sHP, SHORT sMP) {
  ...
  if (gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] > 0) {
    ...
    if ((SHORT)(gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole]) < 0) {
      ...
      gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] = gpGlobals->g.PlayerRoles.rgwMaxHP[wPlayerRole];
    }
    ...
    if ((SHORT)(gpGlobals->g.PlayerRoles.rgwMP[wPlayerRole]) < 0) {
      ...
      gpGlobals->g.PlayerRoles.rgwMP[wPlayerRole] = gpGlobals->g.PlayerRoles.rgwMaxMP[wPlayerRole];
    }
    ...
    if (i <= (short)gpGlobals->wMaxPartyMemberIndex) {
      ...
      for (i = 0; i < MAX_POISONS; i++) {
        ...
        if (gpGlobals->g.rgObject[w].poison.wPoisonLevel < 99) {
          gpGlobals->rgPoisonStatus[j][wPlayerRole] = gpGlobals->rgPoisonStatus[i][wPlayerRole];
          j++;
        }
      }
...
VOID PAL_SetPlayerStatus(WORD wPlayerRole, WORD wStatusID, WORD wNumRound) {
#ifndef PAL_CLASSIC
  if (wStatusID == kStatusSlow && gpGlobals->rgPlayerStatus[wPlayerRole][kStatusHaste] > 0) {
    //
  ...
  if (wStatusID == kStatusHaste && gpGlobals->rgPlayerStatus[wPlayerRole][kStatusSlow] > 0) {
    //
