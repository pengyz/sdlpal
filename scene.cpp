...
static VOID PAL_AddSpriteToDraw(LPCBITMAPRLE lpSpriteFrame, int x, int y, int iLayer)
/*++
   Purpose:
...
static VOID PAL_CalcCoverTiles(SPRITE_TO_DRAW *lpSpriteToDraw)
/*++
   Purpose:
...
--*/
{
  ...
  const int sx =
      PAL_X(gpGlobals->viewport) + PAL_X(lpSpriteToDraw->pos) - lpSpriteToDraw->iLayer / 2;
      ...
      for (i = ((x == (sx - width / 2) / 32) ? 0 : 3); i < 5; i++) {
        ...
        for (l = 0; l < 2; l++) {
          ...
          if (lpTile != NULL && iTileHeight > 0 && (dy + iTileHeight) * 16 + dh * 8 >= sy) {
            ...
            //
            PAL_AddSpriteToDraw(
                lpTile, dx * 32 + dh * 16 - 16 - PAL_X(gpGlobals->viewport),
...
--*/
{
  ...
  for (i = 0; i <= (short)gpGlobals->wMaxPartyMemberIndex + gpGlobals->nFollower; i++) {
    ...
    // Add it to our array
    //
    PAL_AddSpriteToDraw(lpBitmap, gpGlobals->rgParty[i].x - PAL_RLEGetWidth(lpBitmap) / 2,
                        gpGlobals->rgParty[i].y + gpGlobals->wLayer + 10, gpGlobals->wLayer + 6);
  ...
  for (i = gpGlobals->g.rgScene[gpGlobals->wNumScene - 1].wEventObjectIndex;
    ...
    lpFrame = PAL_SpriteGetFrame(lpSprite, lpEvtObj->wDirection * lpEvtObj->nSpriteFrames + iFrame);
...
--*/
{
  ...
  if (fWalking) {
    ...
    for (i = 1; i <= (short)gpGlobals->wMaxPartyMemberIndex; i++) {
      ...
      if (i == 2) {
        gpGlobals->rgParty[i].x += (gpGlobals->rgTrail[1].wDirection == kDirEast ||
                                    gpGlobals->rgTrail[1].wDirection == kDirWest)
                                       ? -16
                                       : 16;
        gpGlobals->rgParty[i].y += 8;
      } else {
        gpGlobals->rgParty[i].x += ((gpGlobals->rgTrail[1].wDirection == kDirWest ||
                                     gpGlobals->rgTrail[1].wDirection == kDirSouth)
                                        ? 16
                                        : -16);
        gpGlobals->rgParty[i].y += ((gpGlobals->rgTrail[1].wDirection == kDirWest ||
                                     gpGlobals->rgTrail[1].wDirection == kDirNorth)
                                        ? 8
                                        : -8);
      }
