...
#define PAL_XY_OFFSET(xy, x, y)                                     \
  (PAL_POS)(((((INT)(y) << 16) & 0xFFFF0000) + ((xy)&0xFFFF0000)) | \
            (((INT)(x)&0xFFFF) + ((xy)&0xFFFF)))
...
typedef enum tagCDTYPE { CD_NONE, CD_MP3, CD_OGG, CD_OPUS, CD_SDLCD } CDTYPE, *LPCDTYPE;
...
INT PAL_RLEBlitToSurfaceWithShadow(LPCBITMAPRLE lpBitmapRLE, SDL_Surface *lpDstSurface, PAL_POS pos,
                                   BOOL bShadow);

INT PAL_RLEBlitWithColorShift(LPCBITMAPRLE lpBitmapRLE, SDL_Surface *lpDstSurface, PAL_POS pos,
                              INT iColorShift);

INT PAL_RLEBlitMonoColor(LPCBITMAPRLE lpBitmapRLE, SDL_Surface *lpDstSurface, PAL_POS pos,
                         BYTE bColor, INT iColorShift);
...
// From yj1.c:
extern INT (*Decompress)(LPCVOID Source, LPVOID Destination, INT DestSize);
