...
LPBOX
PAL_CreateBoxWithShadow(PAL_POS pos, INT nRows, INT nColumns, INT iStyle, BOOL fSaveScreen,
                        INT nShadowOffset)
/*++
  Purpose:
...
--*/
{
  ...
  for (i = 0; i < nRows; i++) {
    ...
    for (j = 0; j < nColumns; j++) {
      ...
      PAL_RLEBlitToSurfaceWithShadow(rglpBorderBitmap[m][n], gpScreen,
                                     PAL_XY(x + nShadowOffset, rect.y + nShadowOffset), TRUE);
...
--*/
{
  ...
  // Draw the shadow
  //
  PAL_RLEBlitToSurfaceWithShadow(lpBitmapLeft, gpScreen,
                                 PAL_XY(rect.x + nShadowOffset, rect.y + nShadowOffset), TRUE);

  rect.x += PAL_RLEGetWidth(lpBitmapLeft);

  for (i = 0; i < nLen; i++) {
    PAL_RLEBlitToSurfaceWithShadow(lpBitmapMid, gpScreen,
                                   PAL_XY(rect.x + nShadowOffset, rect.y + nShadowOffset), TRUE);
    rect.x += PAL_RLEGetWidth(lpBitmapMid);
  }

  PAL_RLEBlitToSurfaceWithShadow(lpBitmapRight, gpScreen,
                                 PAL_XY(rect.x + nShadowOffset, rect.y + nShadowOffset), TRUE);
...
WORD PAL_ReadMenu(LPITEMCHANGED_CALLBACK lpfnMenuItemChanged, LPCMENUITEM rgMenuItem, INT nMenuItem,
  ...
  while (TRUE) {
    ...
    if (g_InputState.dwKeyPress & (kKeyDown | kKeyRight)) {
      ...
      if (rgMenuItem[wCurrentItem].fEnabled) {
        ...
        //
        PAL_DrawText(PAL_GetWord(rgMenuItem[wCurrentItem].wNumWord), rgMenuItem[wCurrentItem].pos,
                     bLabelColor, FALSE, TRUE, FALSE);
      } else {
        PAL_DrawText(PAL_GetWord(rgMenuItem[wCurrentItem].wNumWord), rgMenuItem[wCurrentItem].pos,
                     MENUITEM_COLOR_INACTIVE, FALSE, TRUE, FALSE);
      }
      ...
      if (rgMenuItem[wCurrentItem].fEnabled) {
        ...
        PAL_DrawText(PAL_GetWord(rgMenuItem[wCurrentItem].wNumWord), rgMenuItem[wCurrentItem].pos,
                     MENUITEM_COLOR_SELECTED_INACTIVE, FALSE, TRUE, FALSE);
      }
      //
      // Fix issue #166
      //
...
--*/
INT PAL_TextWidth(LPCWSTR lpszItemText)
...
INT PAL_WordMaxWidth(INT nFirstWord, INT nWordNum)
/*++
  Purpose:

    Calculate the maximal text width of a specific range of words in number of full width
characters.
...
--*/
{
  ...
  while (fgets(buf, 512, fp) != NULL) {
    ...
    *p++ = '\0';
    ...
    pNew = (LPOBJECTDESC)UTIL_calloc(1, sizeof(OBJECTDESC));
