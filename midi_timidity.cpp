...
static VOID TIMIDITY_Close(LPTIMIDITYPLAYER player) {
  if (player->pSong) {
...
static VOID TIMIDITY_FillBuffer(VOID *object, LPBYTE stream, INT len) {
  ...
  if (player->pSong) {
    ...
    if (player->pSong->playing) Timidity_PlaySome(player->pSong, stream, len);
  }
}

static VOID TIMIDITY_Shutdown(VOID *object) {
  LPTIMIDITYPLAYER player = (LPTIMIDITYPLAYER)object;
  if (player) {
...
static BOOL TIMIDITY_Play(VOID *object, INT iNum, BOOL fLoop, FLOAT flFadeTime) {
  LPTIMIDITYPLAYER player = (LPTIMIDITYPLAYER)object;
  ...
  if (iNum > 0) {
    if (gConfig.fIsWIN95) {
      SDL_RWops *rw = SDL_RWFromFile(
          PAL_CombinePath(0, gConfig.pszGamePath,
                          PAL_va(1, "Musics%s%.3d.mid", PAL_NATIVE_PATH_SEPARATOR, iNum)),
          "rb");
      if (rw) {
    ...
    if (!player->pSong) {
      ...
      if ((fp = UTIL_OpenFile("midi.mkf")) != NULL) {
        if ((size = PAL_MKFGetChunkSize(iNum, fp)) > 0 && (buf = (uint8_t *)UTIL_malloc(size))) {
