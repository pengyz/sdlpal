...
VOID PAL_PlayerInfoBox(PAL_POS pos, WORD wPlayerRole, INT iTimeMeter, BYTE bTimeMeterColor,
                       BOOL fUpdate)
/*++
  Purpose:
...
--*/
{
  ...
  // Draw the box
  //
  PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_PLAYERINFOBOX), gpScreen, pos);
  ...
  if (gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] > 0) {
    for (i = 0; i < kStatusAll; i++) {
      if (gpGlobals->rgPlayerStatus[wPlayerRole][i] > 0 && rgwStatusWord[i] != 0) {
    ...
    case kBattleUIActionCoopMagic:
      ...
      for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
        ...
        if (gpGlobals->g.PlayerRoles.rgwHP[w] < gpGlobals->g.PlayerRoles.rgwMaxHP[w] / 5 ||
            ...
            g_Battle.rgPlayer[i].flTimeMeter < 100 || g_Battle.rgPlayer[i].state == kFighterAct) {
          return FALSE;
        }
      }
#else
      {
        int healthyNumber = 0;
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
          if (PAL_IsPlayerHealthy(gpGlobals->rgParty[i].wPlayerRole)) healthyNumber++;
...
#ifdef PAL_CLASSIC
  MENUITEM rgMenuItem[] = {// value   label                     enabled   position
...
VOID PAL_BattleUIShowText(LPCWSTR lpszText, WORD wDuration)
/*++
  Purpose:
...
--*/
{
  ...
  if (gpGlobals->rgPlayerStatus[w][kStatusPuppet] == 0 &&
      gpGlobals->rgPlayerStatus[w][kStatusSleep] == 0 &&
      gpGlobals->rgPlayerStatus[w][kStatusConfused] == 0 && !g_Battle.UI.fAutoAttack &&
      !gpGlobals->fAutoBattle) {
    AUDIO_PlaySound(78);
  }
#endif
}

static VOID PAL_BattleUIUseItem(VOID)
/*++
  Purpose:
...
static VOID PAL_BattleUIThrowItem(VOID)
/*++
  Purpose:
...
--*/
{
  ...
  for (i = 0; i < MAX_PLAYER_MAGICS; i++) {
    ...
    iPower = (SHORT)(gpGlobals->g.lprgMagic[wMagicNum].wBaseDamage) + RandomLong(0, wRandomRange);
...
--*/
{
  ...
  if (g_Battle.UI.fAutoAttack && !gpGlobals->fAutoBattle) {
    ...
    if (g_InputState.dwKeyPress & kKeyMenu) {
      ...
      PAL_DrawText(itemText, PAL_XY(312 - PAL_TextWidth(itemText), 10), MENUITEM_COLOR_CONFIRMED,
                   TRUE, FALSE, FALSE);
    }
  }
  ...
  switch (g_Battle.UI.state) {
    ...
    case kBattleUISelectMove:
      ...
      //
      {
        ...
        } rgItems[] = {{SPRITENUM_BATTLEICON_ATTACK, PAL_XY(27, 140), kBattleUIActionAttack},
        ...
        for (i = 0; i < 4; i++) {
          if (g_Battle.UI.wSelectedAction == i) {
            PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, rgItems[i].iSpriteNum), gpScreen,
                                 rgItems[i].pos);
          } else if (PAL_BattleUIIsActionValid(rgItems[i].action)) {
            PAL_RLEBlitMonoColor(PAL_SpriteGetFrame(gpSpriteUI, rgItems[i].iSpriteNum), gpScreen,
                                 rgItems[i].pos, 0, -4);
          } else {
            PAL_RLEBlitMonoColor(PAL_SpriteGetFrame(gpSpriteUI, rgItems[i].iSpriteNum), gpScreen,
                                 rgItems[i].pos, 0x10, -4);
          }
        }
            ...
            if (g_InputState.dwKeyPress & kKeySearch) {
              switch (g_Battle.UI.wSelectedAction) {
                case 0:
                  ...
                  //
                  g_Battle.UI.wActionType = kBattleActionAttack;
                  if (PAL_PlayerCanAttackAll(
                          gpGlobals->rgParty[g_Battle.UI.wCurPlayerIndex].wPlayerRole)) {
                    g_Battle.UI.state = kBattleUISelectTargetEnemyAll;
                  } else {
                    if (g_Battle.UI.iPrevEnemyTarget != -1)
                      g_Battle.UI.iSelectedIndex = g_Battle.UI.iPrevEnemyTarget;
                ...
                case 3:
                  ...
                  //                  g_iCurMiscMenuItem = 0; //disabled due to not same as both
                  //                  original version
                  break;
              }
              ...
              w = PAL_BattleUIPickAutoMagic(
                  gpGlobals->rgParty[g_Battle.UI.wCurPlayerIndex].wPlayerRole, 60);
              ...
              if (g_Battle.UI.wCurPlayerIndex > 0) {
                ...
                do {
                  ...
                  if (g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.ActionType ==
                  ...
                  } else if (g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.ActionType ==
                             kBattleActionUseItem) {
                    if (gpGlobals->g
                            .rgObject[g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex]
                                          .action.wActionID]
                            .item.wFlags &
                        kItemFlagConsuming) {
                      for (i = 0; i < MAX_INVENTORY; i++) {
                    ...
                    g_Battle.UI.wCurPlayerIndex > 0 &&
                    (gpGlobals->g.PlayerRoles
                             .rgwHP[gpGlobals->rgParty[g_Battle.UI.wCurPlayerIndex].wPlayerRole] ==
                         0 ||
                     gpGlobals->rgPlayerStatus[gpGlobals->rgParty[g_Battle.UI.wCurPlayerIndex]
                                                   .wPlayerRole][kStatusConfused] > 0 ||
                     gpGlobals->rgPlayerStatus[gpGlobals->rgParty[g_Battle.UI.wCurPlayerIndex]
                                                   .wPlayerRole][kStatusSleep] > 0 ||
                     gpGlobals->rgPlayerStatus[gpGlobals->rgParty[g_Battle.UI.wCurPlayerIndex]
                                                   .wPlayerRole][kStatusParalyzed] > 0));
              }
            }
#else
            else if (g_InputState.dwKeyPress & kKeyMenu) {
              ...
              for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
                if (g_Battle.rgPlayer[i].flTimeMeter >= 100) {
                  g_Battle.rgPlayer[i].flTimeMeter +=
                      100;  // HACKHACK: Prevent the time meter from going below 100
            ...
            if (w != 0xFFFF) {
              ...
              switch (w) {
                ...
                case 1:  // item
                  ...
                  //                  g_iCurSubMenuItem = 0; //disabled due to not same as both
                  //                  original version
                  break;
    ...
    case kBattleUISelectTargetEnemy:
      ...
      //
      if (y == 1) {
        //         g_Battle.UI.iPrevEnemyTarget = x;  //disabled due to not same as both original
        //         version
        if (g_Battle.UI.iSelectedIndex == -1)
          g_Battle.UI.iSelectedIndex = x;
        else
          for (g_Battle.UI.iSelectedIndex = 0; g_Battle.UI.iSelectedIndex < MAX_ENEMIES_IN_TEAM;
               g_Battle.UI.iSelectedIndex++)
            if (g_Battle.rgEnemy[g_Battle.UI.iSelectedIndex].wObjectID != 0) break;
        PAL_BattleCommitAction(FALSE);
        break;
      }
#endif
      ...
      if (s_iFrame & 1) {
        ...
        x -= PAL_RLEGetWidth(PAL_SpriteGetFrame(g_Battle.rgEnemy[i].lpSprite,
      ...
      if (g_InputState.dwKeyPress & kKeyMenu) {
        ...
        //         g_Battle.UI.iPrevEnemyTarget = g_Battle.UI.iSelectedIndex; //disabled due to not
        //         same as both original version
        PAL_BattleCommitAction(FALSE);
      ...
      for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
        if (g_Battle.UI.wActionType == kBattleActionMagic) {
          ...
          if (gpGlobals->g.lprgMagic[w].wType == kMagicTypeTrance) {
            if (i != g_Battle.UI.wCurPlayerIndex) continue;
          }
        }
  ...
  if (!SDL_TICKS_PASSED(SDL_GetTicks(), g_Battle.UI.dwMsgShowTime)) {
    ...
    for (i = 0; i < w; i++) len += PAL_CharWidth(g_Battle.UI.szMsg[i]) >> 3;
  ...
  for (i = 0; i < BATTLEUI_MAX_SHOWNUM; i++) {
    if (g_Battle.UI.rgShowNum[i].wNum > 0) {
      if ((SDL_GetTicks() - g_Battle.UI.rgShowNum[i].dwTime) / BATTLE_FRAME_TIME > 10) {
        ...
        PAL_DrawNumber(
            g_Battle.UI.rgShowNum[i].wNum, 5,
            PAL_XY(PAL_X(g_Battle.UI.rgShowNum[i].pos),
                   PAL_Y(g_Battle.UI.rgShowNum[i].pos) -
                       (SDL_GetTicks() - g_Battle.UI.rgShowNum[i].dwTime) / BATTLE_FRAME_TIME),
