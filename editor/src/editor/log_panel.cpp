...
{
    LogPanel* panel = (LogPanel*)data->UserData;
    return panel->TextEditCallback(data);
}

LogPanel::LogPanel(Window* parent, int width, int height, const std::string& title, bool visible, engine::PalEngine* engine)
...
void LogPanel::render()
{
    ...
    if (!ImGui::Begin(_title.c_str(), &_visible)) {
        ImGui::End();
        return;
    }
    ...
    if (ImGui::BeginChild("ScrollingRegion", ImVec2(0, -footer_height_to_reserve), ImGuiChildFlags_None, ImGuiWindowFlags_HorizontalScrollbar)) {
        ...
        for (const char* item : Items) {
            ...
            if (strstr(item, "[debug]")) {
                color = ImVec4(0.4f, 0.4f, 0.8f, 1.0f);
                has_color = true;
            }
            if (strstr(item, "[error]")) {
                ...
                color = ImGui::GetStyleColorVec4(ImGuiCol_PlotHistogram);
                has_color = true;
...
void LogPanel::ExecCommand(const char* command_line)
{
    AddLog("# %s\n", command_line);
...
int LogPanel::TextEditCallback(ImGuiInputTextCallbackData* data)
{
