...
#define SDL_OpenAudio(desired, obtained)                                                         \
  ((gAudioDevice.id = SDL_OpenAudioDevice(                                                       \
        (gConfig.iAudioDevice >= 0 ? SDL_GetAudioDeviceName(gConfig.iAudioDevice, 0) : NULL), 0, \
        (desired), (obtained), 0)) > 0                                                           \
       ? gAudioDevice.id                                                                         \
       : -1)
#endif
...
static VOID SDLCALL AUDIO_FillBuffer(LPVOID udata, LPBYTE stream, INT len)
/*++
  Purpose:
...
--*/
{
  ...
  if (gAudioDevice.fSoundEnabled && gAudioDevice.pSoundPlayer && gAudioDevice.iSoundVolume > 0) {
    ...
    gAudioDevice.pSoundPlayer->FillBuffer(gAudioDevice.pSoundPlayer,
                                          (LPBYTE)gAudioDevice.pSoundBuffer, len);
...
--*/
{
  ...
  if (gConfig.eMIDISynth == SYNTH_NATIVE) MIDI_SetVolume(gConfig.iMusicVolume);
  ...
  UTIL_LogOutput(
      LOGLEVEL_VERBOSE, "OpenAudio: requesting audio device: %s\n",
  ...
  UTIL_LogOutput(LOGLEVEL_VERBOSE,
                 "OpenAudio: requesting audio spec:freq %d, format %d, channels %d, samples %d\n",
  ...
  if (SDL_OpenAudio(&gAudioDevice.spec, &spec) < 0) {
    UTIL_LogOutput(LOGLEVEL_VERBOSE,
                   "OpenAudio ERROR: %s, got spec:freq %d, format %d, channels %d, samples %d\n",
    ...
    UTIL_LogOutput(LOGLEVEL_VERBOSE,
                   "OpenAudio succeed, got spec:freq %d, format %d, channels %d, samples %d\n",
                   spec.freq, spec.format, spec.channels, spec.samples);
    gAudioDevice.pSoundBuffer =
        malloc(gConfig.wAudioBufferSize * gConfig.iAudioChannels * sizeof(short));
  }
  ...
  switch (gConfig.eMusicType) {
    case MUSIC_RIX:
      gAudioDevice.pMusPlayer =
          RIX_Init(UTIL_GetFullPathName(PAL_BUFFER_SIZE_ARGS(0), gConfig.pszGamePath, "mus.mkf"));
      break;
    case MUSIC_MP3:
...
static INT AUDIO_ChangeVolumeByValue(INT *iVolume, INT iValue) {
  *iVolume += iValue;
...
--*/
{
  ...
  if (gAudioDevice.pCDPlayer) {
    if (iNumTrack != -1) {
      ret = gAudioDevice.pCDPlayer->Play(gAudioDevice.pCDPlayer, PAL_CDTRACK_BASE + iNumTrack, TRUE,
                                         0);
    } else {
