...
--*/
{
  ...
  if (fTrigger) {
    ...
    for (wEventObjectID = gpGlobals->g.rgScene[gpGlobals->wNumScene - 1].wEventObjectIndex + 1;
      ...
      if (p->sState < 0) {
        if (p->x < PAL_X(gpGlobals->viewport) || p->x > PAL_X(gpGlobals->viewport) + 320 ||
            p->y < PAL_Y(gpGlobals->viewport) || p->y > PAL_Y(gpGlobals->viewport) + 320) {
  ...
  for (wEventObjectID = gpGlobals->g.rgScene[gpGlobals->wNumScene - 1].wEventObjectIndex + 1;
    ...
    if (fTrigger && p->sState >= kObjStateBlocker && p->wSpriteNum != 0 &&
        abs(p->x - PAL_X(gpGlobals->viewport) - PAL_X(gpGlobals->partyoffset)) +
                abs(p->y - PAL_Y(gpGlobals->viewport) - PAL_Y(gpGlobals->partyoffset)) * 2 <=
            12) {
      //
      ...
      for (i = 0; i < 4; i++) {
        ...
        if (!PAL_CheckObstacle(pos, TRUE, 0)) {
          ...
          // move here
          //
          gpGlobals->viewport = PAL_XY(PAL_X(pos) - PAL_X(gpGlobals->partyoffset),
  ...
  while (TRUE) {
    ...
    if (!(gpGlobals->g.rgObject[wObject].item.wFlags & kItemFlagApplyToAll)) {
      ...
      while (TRUE) {
        ...
        //
        if ((gpGlobals->g.rgObject[wObject].item.wFlags & kItemFlagConsuming) && g_fScriptSuccess) {
...
--*/
{
  ...
  for (i = 0; i < 13; i++) {
    ...
    for (k = gpGlobals->g.rgScene[gpGlobals->wNumScene - 1].wEventObjectIndex;
      ...
      if (p->sState <= 0 || p->wTriggerMode >= kTriggerTouchNear || p->wTriggerMode * 6 - 4 < i ||
          dx != ex || dy != ey || dh != eh) {
        continue;
      }
