...
--*/
{
  ...
  if (i >= 0 && i <= g_Battle.wMaxEnemyIndex && g_Battle.rgEnemy[i].wObjectID != 0 &&
...
static SHORT PAL_CalcMagicDamage(WORD wMagicStrength, WORD wDefense,
                                 ...
                                 WORD wPoisonResistance, WORD wResistanceMultiplier, WORD wMagicID)
/*++
   Purpose:
...
--*/
{
  ...
  if (gpGlobals->g.lprgMagic[wMagicID].wElemental != 0) {
    ...
    if (wElem <= NUM_MAGIC_ELEMENTAL) {
      sDamage *=
          10 + gpGlobals->g.lprgBattleField[gpGlobals->wNumBattleField].rgsMagicEffect[wElem - 1];
      sDamage /= 10;
    }
  }
...
SHORT
PAL_CalcPhysicalAttackDamage(WORD wAttackStrength, WORD wDefense, WORD wAttackResistance)
/*++
  Purpose:
      ...
      for (j = 0; j <= g_Battle.wMaxEnemyIndex; j++) {
        ...
        if (--g_Battle.rgEnemy[j].e.wIdleAnimSpeed == 0) {
          ...
          g_Battle.rgEnemy[j].e.wIdleAnimSpeed =
              gpGlobals->g
                  .lprgEnemy[gpGlobals->g.rgObject[g_Battle.rgEnemy[j].wObjectID].enemy.wEnemyID]
                  .wIdleAnimSpeed;
        }
  ...
  for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
    ...
    if (--g_Battle.rgEnemy[i].e.wIdleAnimSpeed == 0) {
      ...
      g_Battle.rgEnemy[i].e.wIdleAnimSpeed =
          gpGlobals->g
              .lprgEnemy[gpGlobals->g.rgObject[g_Battle.rgEnemy[i].wObjectID].enemy.wEnemyID]
              .wIdleAnimSpeed;
    }
  ...
  for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
    if (g_Battle.rgPlayer[i].state == kFighterCom ||
        (g_Battle.rgPlayer[i].state == kFighterAct &&
         g_Battle.rgPlayer[i].action.ActionType == kBattleActionCoopMagic)) {
      flMax = 0;
      break;
...
--*/
{
  ...
  for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
    ...
    switch (g_Battle.rgEnemy[i].state) {
      ...
      case kFighterAct:
        if (!fMoved && (PAL_GetTimeChargingSpeed(9999) > 0 || g_Battle.rgEnemy[i].fDualMove) &&
            !fOnlyPuppet) {
          fMoved = TRUE;
  ...
  if (!fMoved) {
    ...
    for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
      ...
      if (g_Battle.rgPlayer[i].state == kFighterAct && g_Battle.rgPlayer[i].sTurnOrder != -1 &&
  ...
  if (g_Battle.Phase == kBattlePhaseSelectAction) {
      ...
      if (i > gpGlobals->wMaxPartyMemberIndex) {
          ...
          if (gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] == 0 ||
            ...
            switch (g_Battle.rgPlayer[i].action.ActionType) {
              ...
              case kBattleActionMagic:
                if ((gpGlobals->g.rgObject[g_Battle.rgPlayer[i].action.wActionID].magic.wFlags &
                     kMagicFlagUsableToEnemy) == 0) {
                  wDexterity *= 3;
                }
                break;
        ...
        for (i = 0; i < MAX_ACTIONQUEUE_ITEMS; i++) {
          for (j = i; j < MAX_ACTIONQUEUE_ITEMS; j++) {
            if ((SHORT)g_Battle.ActionQueue[i].wDexterity <
                (SHORT)g_Battle.ActionQueue[j].wDexterity) {
    ...
    if (g_Battle.iCurAction >= MAX_ACTIONQUEUE_ITEMS ||
      ...
      if (g_Battle.ActionQueue[g_Battle.iCurAction].fIsEnemy) {
        if (g_Battle.iHidingTime == 0 && !fOnlyPuppet && g_Battle.rgEnemy[i].wObjectID != 0) {
          g_Battle.rgEnemy[i].wScriptOnReady =
              PAL_RunTriggerScript(g_Battle.rgEnemy[i].wScriptOnReady, i);
...
--*/
{
  ...
  if (!fRepeat) {
    ...
    memset(&g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action, 0,
           sizeof(g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action));
    g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.ActionType =
        (BATTLEACTIONTYPE)g_Battle.UI.wActionType;
    ...
    if (g_Battle.UI.wActionType == kBattleActionAttack) {
      ...
      g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.wActionID = g_Battle.UI.wObjectID;
    }
#ifndef PAL_CLASSIC
  ...
  switch (g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.ActionType) {
    case kBattleActionMagic:
      ...
      if (gpGlobals->g.PlayerRoles
              .rgwMP[gpGlobals->rgParty[g_Battle.UI.wCurPlayerIndex].wPlayerRole] < w) {
        w = g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.wActionID;
        w = gpGlobals->g.lprgMagic[gpGlobals->g.rgObject[w].magic.wMagicNumber].wType;
        if (w == kMagicTypeApplyToPlayer || w == kMagicTypeApplyToParty || w == kMagicTypeTrance) {
...
#ifdef PAL_CLASSIC
    case kBattleActionUseItem:
      if ((gpGlobals->g.rgObject[g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.wActionID]
               .item.wFlags &
           kItemFlagConsuming) == 0) {
        break;
      }

    case kBattleActionThrowItem:
      for (w = 0; w < MAX_INVENTORY; w++) {
        if (gpGlobals->rgInventory[w].wItem ==
            g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.wActionID) {
...
--*/
{
  ...
  if (sTarget != -1) {
    ...
    enemy_h = PAL_RLEGetHeight(PAL_SpriteGetFrame(g_Battle.rgEnemy[sTarget].lpSprite,
                                                  g_Battle.rgEnemy[sTarget].wCurrentFrame));
  ...
  for (i = 0; i < 3; i++) {
    ...
    //
    for (j = 0; j <= g_Battle.wMaxEnemyIndex; j++) {
      if (g_Battle.rgEnemy[j].wObjectID == 0 || g_Battle.rgEnemy[j].rgwStatus[kStatusSleep] > 0 ||
          g_Battle.rgEnemy[j].rgwStatus[kStatusParalyzed] > 0) {
        continue;
      }
    ...
    if (i == 1) {
      g_Battle.rgPlayer[wPlayerIndex].pos = PAL_XY(PAL_X(g_Battle.rgPlayer[wPlayerIndex].pos) + 2,
...
--*/
{
  ...
  g_Battle.rgPlayer[wPlayerIndex].pos = PAL_XY(PAL_X(g_Battle.rgPlayer[wPlayerIndex].pos) - 15,
  ...
  for (i = 0; i < n; i++) {
    ...
    if (gpGlobals->g.lprgMagic[iMagicNum].wType == kMagicTypeApplyToParty) {
      ...
      if (sTarget > 0 && g_Battle.iHidingTime == 0) {
        if (gpGlobals->rgPlayerStatus[gpGlobals->rgParty[sTarget - 1].wPlayerRole]
                                     [kStatusConfused] == 0) {
...
static VOID PAL_BattleShowPlayerOffMagicAnim(WORD wPlayerIndex, WORD wObjectID, SHORT sTarget,
                                             BOOL fSummon)
/*++
  Purpose:
...
--*/
{
  ...
  for (i = 0; i < l; i++) {
    LPCBITMAPRLE b;
    if (!gConfig.fIsWIN95 && i == gpGlobals->g.lprgMagic[iMagicNum].wFireDelay &&
        wPlayerIndex != (WORD)-1) {
      g_Battle.rgPlayer[wPlayerIndex].wCurrentFrame = 6;
    }
    ...
    if (l - i > gpGlobals->g.lprgMagic[iMagicNum].wShake) {
      ...
      b = PAL_SpriteGetFrame(lpSpriteEffect,
                             (l - gpGlobals->g.lprgMagic[iMagicNum].wShake - 1) % n);
    }
  ...
  for (i = 0; i < l; i++) {
    ...
    if (l - i > gpGlobals->g.lprgMagic[iMagicNum].wShake) {
      ...
      if (gpGlobals->g.lprgMagic[iMagicNum].wFireDelay > 0 &&
          ...
          i < gpGlobals->g.lprgMagic[iMagicNum].wFireDelay +
                  g_Battle.rgEnemy[wEnemyIndex].e.wAttackFrames) {
        g_Battle.rgEnemy[wEnemyIndex].wCurrentFrame = i -
                                                      gpGlobals->g.lprgMagic[iMagicNum].wFireDelay +
                                                      g_Battle.rgEnemy[wEnemyIndex].e.wIdleFrames +
                                                      g_Battle.rgEnemy[wEnemyIndex].e.wMagicFrames;
      }
    } else {
      VIDEO_ShakeScreen(i, 3);
      b = PAL_SpriteGetFrame(lpSpriteEffect,
...
--*/
{
  ...
  g_Battle.lpSummonSprite = (LPSPRITE)UTIL_malloc(i);
...
--*/
{
  ...
  switch (g_Battle.rgPlayer[wPlayerIndex].action.ActionType) {
    ...
    case kBattleActionMagic:
      ...
      if (gpGlobals->g.PlayerRoles.rgwMP[wPlayerRole] < gpGlobals->g.lprgMagic[w].wCostMP) {
        //
        // No enough MP
        //
        fValid = FALSE;
      ...
      if (gpGlobals->g.rgObject[wObjectID].magic.wFlags & kMagicFlagUsableToEnemy) {
        if (!fValid) {
          ...
          g_Battle.rgPlayer[wPlayerIndex].action.sTarget =
              PAL_BattleSelectAutoTargetFrom(g_Battle.rgPlayer[wPlayerIndex].action.sTarget);
        }
    ...
    case kBattleActionCoopMagic:
...
#ifdef PAL_CLASSIC
      {
        ...
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
          ...
          if (g_Battle.coopContributors[i]) iTotalHealthy++;
        }
        if (iTotalHealthy <= 1) {
          g_Battle.rgPlayer[wPlayerIndex].action.ActionType = kBattleActionAttack;
          g_Battle.rgPlayer[wPlayerIndex].action.wActionID = 0;
      ...
      for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
        ...
        if (PAL_IsPlayerDying(w) || gpGlobals->rgPlayerStatus[w][kStatusSilence] > 0 ||
            gpGlobals->rgPlayerStatus[w][kStatusSleep] > 0 ||
            gpGlobals->rgPlayerStatus[w][kStatusConfused] > 0 ||
    ...
    case kBattleActionAttackMate:
      if (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusConfused] == 0) {
        ...
        if (i > gpGlobals->wMaxPartyMemberIndex) {
          //
          // DISABLE Attack enemies if no one else is alive; since original version behaviour is not
          // same
          //
          //            fToEnemy = TRUE;
...
INT FIGHT_DetectMagicTargetChange(WORD wMagicNum, INT sTarget) {
  if (sTarget == -1 && (gpGlobals->g.lprgMagic[wMagicNum].wType == kMagicTypeNormal ||
                        gpGlobals->g.lprgMagic[wMagicNum].wType == kMagicTypeApplyToPlayer ||
                        gpGlobals->g.lprgMagic[wMagicNum].wType == kMagicTypeTrance))
    sTarget = 0;

  if (sTarget != -1 && (gpGlobals->g.lprgMagic[wMagicNum].wType == kMagicTypeAttackAll ||
...
--*/
{
  ...
  switch (g_Battle.rgPlayer[wPlayerIndex].action.ActionType) {
    case kBattleActionAttack:
#ifdef PAL_CLASSIC
      if (g_Battle.fThisTurnCoop) break;
#endif
      if (sTarget != -1) {
        //
        // Attack one enemy
        ...
        for (t = 0; t < (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusDualAttack] ? 2 : 1); t++) {
          ...
          if (RandomLong(0, 5) == 0 || gpGlobals->rgPlayerStatus[wPlayerRole][kStatusBravery] > 0) {
            //
            // Critical Hit
            //
            sDamage *= 3;
        ...
        for (t = 0; t < (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusDualAttack] ? 2 : 1); t++) {
          ...
          for (i = 0; i < MAX_ENEMIES_IN_TEAM; i++) {
            if (g_Battle.rgEnemy[index[i]].wObjectID == 0 || index[i] > g_Battle.wMaxEnemyIndex) {
              continue;
            }
    ...
    case kBattleActionAttackMate:
      ...
      if (i <= gpGlobals->wMaxPartyMemberIndex) {
        ...
        } while (sTarget == wPlayerIndex ||
                 gpGlobals->g.PlayerRoles.rgwHP[gpGlobals->rgParty[sTarget].wPlayerRole] == 0);
        ...
        if (gpGlobals->rgPlayerStatus[gpGlobals->rgParty[sTarget].wPlayerRole][kStatusProtect] >
            0) {
          sDamage /= 2;
        }
        ...
        if (sDamage >
            (SHORT)gpGlobals->g.PlayerRoles.rgwHP[gpGlobals->rgParty[sTarget].wPlayerRole]) {
        ...
        g_Battle.rgPlayer[sTarget].pos = PAL_XY(PAL_X(g_Battle.rgPlayer[sTarget].pos) - 12,
    ...
    case kBattleActionCoopMagic:
...
#ifdef PAL_CLASSIC
            if (g_Battle.coopContributors[j] == FALSE) continue;
#endif
...
#ifdef PAL_CLASSIC
          if (g_Battle.coopContributors[i] == FALSE) continue;
#endif
      ...
      if (sTarget == -1) {
        ...
        for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
          ...
          sDamage = PAL_CalcMagicDamage(str, def, g_Battle.rgEnemy[i].e.wElemResistance,
                                        g_Battle.rgEnemy[i].e.wPoisonResistance, 1, wObject);
        ...
        sDamage = PAL_CalcMagicDamage(str, def, g_Battle.rgEnemy[sTarget].e.wElemResistance,
                                      g_Battle.rgEnemy[sTarget].e.wPoisonResistance, 1, wObject);
    ...
    case kBattleActionMagic:
      ...
      PAL_BattleShowPlayerPreMagicAnim(
          wPlayerIndex, (gpGlobals->g.lprgMagic[wMagicNum].wType == kMagicTypeSummon));
      ...
      if (gpGlobals->g.lprgMagic[wMagicNum].wType == kMagicTypeApplyToPlayer ||
        ...
        if (g_fScriptSuccess) {
          ...
          gpGlobals->g.rgObject[wObject].magic.wScriptOnSuccess = PAL_RunTriggerScript(
              gpGlobals->g.rgObject[wObject].magic.wScriptOnSuccess, (WORD)sTarget);
          ...
          if ((SHORT)(gpGlobals->g.lprgMagic[wMagicNum].wBaseDamage) > 0) {
            if (sTarget == -1) {
              ...
              sDamage =
                  PAL_CalcMagicDamage(str, def, g_Battle.rgEnemy[sTarget].e.wElemResistance,
                                      g_Battle.rgEnemy[sTarget].e.wPoisonResistance, 1, wObject);
  ...
  if (fCheckPoison) {
    ...
    for (i = 0; i < MAX_POISONS; i++) {
      ...
      if (wObject != 0) {
        fPoisoned = TRUE;
        gpGlobals->rgPoisonStatus[i][wPlayerIndex].wPoisonScript = PAL_RunTriggerScript(
            gpGlobals->rgPoisonStatus[i][wPlayerIndex].wPoisonScript, wPlayerRole);
      }
    }
...
--*/
{
  ...
  if (g_Battle.rgEnemy[wEnemyIndex].rgwStatus[kStatusSleep] > 0 ||
      g_Battle.rgEnemy[wEnemyIndex].rgwStatus[kStatusParalyzed] > 0 || g_Battle.iHidingTime > 0) {
    //
    // Do nothing
    //
    goto end;
    ...
    if (iTarget == wEnemyIndex) goto end;
    ...
    y = PAL_Y(g_Battle.rgEnemy[iTarget].pos) -
        PAL_RLEGetHeight(PAL_SpriteGetFrame(g_Battle.rgEnemy[iTarget].lpSprite, 0)) / 3 + 10;
    for (i = 9; i < 12; i++) {
    ...
    PAL_BattlePostActionCheck(FALSE);
  } else if (wMagic != 0 && RandomLong(0, 9) < g_Battle.rgEnemy[wEnemyIndex].e.wMagicRate &&
    ...
    for (i = 0; i < g_Battle.rgEnemy[wEnemyIndex].e.wMagicFrames; i++) {
      g_Battle.rgEnemy[wEnemyIndex].wCurrentFrame = g_Battle.rgEnemy[wEnemyIndex].e.wIdleFrames + i;
    ...
    if (gpGlobals->g.lprgMagic[wMagicNum].wFireDelay == 0) {
      for (i = 0; i <= g_Battle.rgEnemy[wEnemyIndex].e.wAttackFrames; i++) {
        g_Battle.rgEnemy[wEnemyIndex].wCurrentFrame = i - 1 +
                                                      g_Battle.rgEnemy[wEnemyIndex].e.wIdleFrames +
                                                      g_Battle.rgEnemy[wEnemyIndex].e.wMagicFrames;
        PAL_BattleDelay(g_Battle.rgEnemy[wEnemyIndex].e.wActWaitFrames, 0, FALSE);
      }
...
#endif
            gpGlobals->rgPlayerStatus[w][kStatusConfused] == 0 && RandomLong(0, 2) == 0 &&
            gpGlobals->g.PlayerRoles.rgwHP[w] != 0) {
    ...
    if ((SHORT)(gpGlobals->g.lprgMagic[wMagicNum].wBaseDamage) > 0) {
      if (sTarget == -1) {
        ...
        for (x = 0; x < NUM_MAGIC_ELEMENTAL; x++) {
          rgwElementalResistance[x] = 100 + PAL_GetPlayerElementalResistance(wPlayerRole, x);
        }
    ...
    for (i = 0; i < 5; i++) {
      if (sTarget == -1) {
        for (x = 0; x <= gpGlobals->wMaxPartyMemberIndex; x++) {
          ...
          if (i > 0) {
            g_Battle.rgPlayer[x].pos = PAL_XY(PAL_X(g_Battle.rgPlayer[x].pos) + (8 >> i),
        ...
        if (i > 0) {
          g_Battle.rgPlayer[sTarget].pos = PAL_XY(PAL_X(g_Battle.rgPlayer[sTarget].pos) + (8 >> i),
    ...
    if (gConfig.fIsWIN95 && g_Battle.ActionQueue[g_Battle.iCurAction].fIsSecond &&
        g_Battle.rgEnemy[wEnemyIndex].e.wMagic == 0)
      AUDIO_PlaySound(g_Battle.rgEnemy[wEnemyIndex].e.wMagicSound);
    else
    ...
    if ((PAL_IsPlayerDying(wPlayerRole) ||
      ...
      if (iCoverIndex != -1) {
        if (PAL_IsPlayerDying(gpGlobals->rgParty[iCoverIndex].wPlayerRole) ||
            gpGlobals->rgPlayerStatus[gpGlobals->rgParty[iCoverIndex].wPlayerRole]
                                     [kStatusConfused] > 0 ||
            gpGlobals->rgPlayerStatus[gpGlobals->rgParty[iCoverIndex].wPlayerRole][kStatusSleep] >
                0 ||
            gpGlobals->rgPlayerStatus[gpGlobals->rgParty[iCoverIndex].wPlayerRole]
                                     [kStatusParalyzed] > 0) {
          iCoverIndex = -1;
    ...
    //
    if (iCoverIndex == -1 && (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusConfused] > 0 ||
                              gpGlobals->rgPlayerStatus[wPlayerRole][kStatusSleep] > 0 ||
#ifdef PAL_CLASSIC
                              gpGlobals->rgPlayerStatus[wPlayerRole][kStatusParalyzed] > 0))
#else
    ...
    if (g_Battle.rgEnemy[wEnemyIndex].e.wAttackFrames == 0) {
      g_Battle.rgEnemy[wEnemyIndex].wCurrentFrame = g_Battle.rgEnemy[wEnemyIndex].e.wIdleFrames - 1;
      ...
      for (i = 0; i <= g_Battle.rgEnemy[wEnemyIndex].e.wAttackFrames; i++) {
        g_Battle.rgEnemy[wEnemyIndex].wCurrentFrame = g_Battle.rgEnemy[wEnemyIndex].e.wIdleFrames +
                                                      g_Battle.rgEnemy[wEnemyIndex].e.wMagicFrames +
                                                      i - 1;
    ...
    if (iCoverIndex != -1) {
      g_Battle.rgEnemy[wEnemyIndex].pos = PAL_XY(PAL_X(g_Battle.rgEnemy[wEnemyIndex].pos) - 10,
                                                 PAL_Y(g_Battle.rgEnemy[wEnemyIndex].pos) - 8);
      g_Battle.rgPlayer[iCoverIndex].pos = PAL_XY(PAL_X(g_Battle.rgPlayer[iCoverIndex].pos) + 4,
                                                  PAL_Y(g_Battle.rgPlayer[iCoverIndex].pos) + 2);
    } else {
      g_Battle.rgPlayer[sTarget].pos = PAL_XY(PAL_X(g_Battle.rgPlayer[sTarget].pos) + 8,
    ...
    if (iCoverIndex == -1) {
      g_Battle.rgPlayer[sTarget].pos = PAL_XY(PAL_X(g_Battle.rgPlayer[sTarget].pos) + 2,
  ...
  if (!g_Battle.rgEnemy[wEnemyIndex].fDualMove) {
    ...
    for (i = 0; i < MAX_POISONS; i++) {
      if (g_Battle.rgEnemy[wEnemyIndex].rgPoisons[i].wPoisonID != 0) {
        g_Battle.rgEnemy[wEnemyIndex].rgPoisons[i].wPoisonScript = PAL_RunTriggerScript(
            g_Battle.rgEnemy[wEnemyIndex].rgPoisons[i].wPoisonScript, wEnemyIndex);
      }
    }
...
--*/
{
  ...
  if (g_Battle.rgEnemy[wTarget].e.nStealItem > 0 &&
    ...
    if (g_Battle.rgEnemy[wTarget].e.wStealItem == 0) {
      ...
      if (c > 0) {
#ifdef PAL_CLASSIC
        PAL_swprintf(s, sizeof(s) / sizeof(WCHAR), L"@%ls @%d @%ls@", PAL_GetWord(34), c,
                     PAL_GetWord(10));
#else
        PAL_swprintf(s, sizeof(s) / sizeof(WCHAR), L"%ls %d %ls", PAL_GetWord(34), c,
                     PAL_GetWord(10));
...
#ifdef PAL_CLASSIC
      PAL_swprintf(s, sizeof(s) / sizeof(WCHAR), L"%ls@%ls@", PAL_GetWord(34),
                   PAL_GetWord(g_Battle.rgEnemy[wTarget].e.wStealItem));
#else
      PAL_swprintf(s, sizeof(s) / sizeof(WCHAR), L"%ls %ls", PAL_GetWord(34),
                   PAL_GetWord(g_Battle.rgEnemy[wTarget].e.wStealItem));
#endif
...
--*/
{
  ...
  if (gpGlobals->g.lprgMagic[gpGlobals->g.rgObject[wMagicObjectID].magic.wMagicNumber].wBaseDamage >
          0 ||
      wBaseDamage > 0) {
    if (sTarget == -1) {
      //
      ...
      sDamage =
          PAL_CalcMagicDamage(wBaseDamage, (WORD)def, g_Battle.rgEnemy[sTarget].e.wElemResistance,
