...
--*/
{
  ...
  // sort by y
  for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++) enemyDrawSeq[i] = i;
  ...
  for (j = g_Battle.wMaxEnemyIndex; j >= 0; j--) {
    ...
    pos =
        PAL_XY(PAL_X(pos) - PAL_RLEGetWidth(PAL_SpriteGetFrame(g_Battle.rgEnemy[i].lpSprite,
                                                               g_Battle.rgEnemy[i].wCurrentFrame)) /
                                2,
               PAL_Y(pos) - PAL_RLEGetHeight(PAL_SpriteGetFrame(
                                g_Battle.rgEnemy[i].lpSprite, g_Battle.rgEnemy[i].wCurrentFrame)));

    if (g_Battle.rgEnemy[i].wObjectID != 0) {
      if (g_Battle.rgEnemy[i].iColorShift) {
        PAL_RLEBlitWithColorShift(
            PAL_SpriteGetFrame(g_Battle.rgEnemy[i].lpSprite, g_Battle.rgEnemy[i].wCurrentFrame),
  ...
  if (g_Battle.lpSummonSprite != NULL) {
    ...
    for (i = gpGlobals->wMaxPartyMemberIndex; i >= 0; i--) {
      ...
      pos = PAL_XY(
          PAL_X(pos) - PAL_RLEGetWidth(PAL_SpriteGetFrame(g_Battle.rgPlayer[i].lpSprite,
                                                          g_Battle.rgPlayer[i].wCurrentFrame)) /
                           2,
          PAL_Y(pos) - PAL_RLEGetHeight(PAL_SpriteGetFrame(g_Battle.rgPlayer[i].lpSprite,
                                                           g_Battle.rgPlayer[i].wCurrentFrame)));

      if (g_Battle.rgPlayer[i].iColorShift != 0) {
        PAL_RLEBlitWithColorShift(
            PAL_SpriteGetFrame(g_Battle.rgPlayer[i].lpSprite, g_Battle.rgPlayer[i].wCurrentFrame),
    ...
    for (i = gpGlobals->wMaxPartyMemberIndex; i >= 0; i--) {
      if (gpGlobals->rgPlayerStatus[gpGlobals->rgParty[i].wPlayerRole][kStatusConfused] != 0 &&
        ...
        if (!PAL_IsPlayerDying(gpGlobals->rgParty[i].wPlayerRole)) yd += RandomLong(-1, 1);
        pos = PAL_XY(xd, yd);
        pos = PAL_XY(
            PAL_X(pos) - PAL_RLEGetWidth(PAL_SpriteGetFrame(g_Battle.rgPlayer[i].lpSprite,
                                                            g_Battle.rgPlayer[i].wCurrentFrame)) /
...
--*/
{
  ...
  int maxNameWidth =
      PAL_MenuTextMaxWidth(rgFakeMenuItem, sizeof(rgFakeMenuItem) / sizeof(MENUITEM));
  ...
  int maxPropertyWidth =
      PAL_MenuTextMaxWidth(rgFakeMenuItem2, sizeof(rgFakeMenuItem2) / sizeof(MENUITEM)) - 1;
  ...
  for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
    ...
    if (fLevelUp) {
      ...
      WCHAR buffer[256] = L"";
      PAL_swprintf(buffer, sizeof(buffer) / sizeof(WCHAR), L"%ls%ls%ls",
      ...
      for (j = 0; j < 8; j++) {
        PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_ARROW), gpScreen,
                             PAL_XY(-offsetX + 180, 48 + 18 * j));
      }

      PAL_DrawText(PAL_GetWord(STATUS_LABEL_LEVEL), PAL_XY(offsetX + 100, 44),
                   BATTLEWIN_LEVELUP_LABEL_COLOR, TRUE, FALSE, FALSE);
      PAL_DrawText(PAL_GetWord(STATUS_LABEL_HP), PAL_XY(offsetX + 100, 62),
                   BATTLEWIN_LEVELUP_LABEL_COLOR, TRUE, FALSE, FALSE);
      PAL_DrawText(PAL_GetWord(STATUS_LABEL_MP), PAL_XY(offsetX + 100, 80),
                   BATTLEWIN_LEVELUP_LABEL_COLOR, TRUE, FALSE, FALSE);
      PAL_DrawText(PAL_GetWord(STATUS_LABEL_ATTACKPOWER), PAL_XY(offsetX + 100, 98),
                   BATTLEWIN_LEVELUP_LABEL_COLOR, TRUE, FALSE, FALSE);
      PAL_DrawText(PAL_GetWord(STATUS_LABEL_MAGICPOWER), PAL_XY(offsetX + 100, 116),
                   BATTLEWIN_LEVELUP_LABEL_COLOR, TRUE, FALSE, FALSE);
      PAL_DrawText(PAL_GetWord(STATUS_LABEL_RESISTANCE), PAL_XY(offsetX + 100, 134),
                   BATTLEWIN_LEVELUP_LABEL_COLOR, TRUE, FALSE, FALSE);
      PAL_DrawText(PAL_GetWord(STATUS_LABEL_DEXTERITY), PAL_XY(offsetX + 100, 152),
                   BATTLEWIN_LEVELUP_LABEL_COLOR, TRUE, FALSE, FALSE);
      PAL_DrawText(PAL_GetWord(STATUS_LABEL_FLEERATE), PAL_XY(offsetX + 100, 170),
                   BATTLEWIN_LEVELUP_LABEL_COLOR, TRUE, FALSE, FALSE);

      //
      // Draw the original stats and stats after level up
      //
      PAL_DrawNumber(OrigPlayerRoles.rgwLevel[w], 4, PAL_XY(-offsetX + 133, 47), kNumColorYellow,
                     kNumAlignRight);
      PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwLevel[w], 4, PAL_XY(-offsetX + 195, 47),
                     kNumColorYellow, kNumAlignRight);

      PAL_DrawNumber(OrigPlayerRoles.rgwHP[w], 4, PAL_XY(-offsetX + 133, 64), kNumColorYellow,
                     kNumAlignRight);
      ...
      PAL_DrawNumber(OrigPlayerRoles.rgwMP[w], 4, PAL_XY(-offsetX + 133, 82), kNumColorYellow,
                     kNumAlignRight);
      PAL_DrawNumber(OrigPlayerRoles.rgwMaxMP[w], 4, PAL_XY(-offsetX + 154, 86), kNumColorBlue,
                     kNumAlignRight);
      PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen,
      ...
      PAL_DrawNumber(PAL_GetPlayerMagicStrength(w), 4, PAL_XY(-offsetX + 195, 119), kNumColorYellow,
                     kNumAlignRight);
      ...
      PAL_DrawNumber(PAL_GetPlayerDefense(w), 4, PAL_XY(-offsetX + 195, 137), kNumColorYellow,
                     kNumAlignRight);
      ...
      PAL_DrawNumber(PAL_GetPlayerDexterity(w), 4, PAL_XY(-offsetX + 195, 155), kNumColorYellow,
                     kNumAlignRight);
      ...
      PAL_DrawNumber(PAL_GetPlayerFleeRate(w), 4, PAL_XY(-offsetX + 195, 173), kNumColorYellow,
                     kNumAlignRight);
...
#define CHECK_HIDDEN_EXP(expname, statname, label)                                                 \
  {                                                                                                \
    ...
    if (gpGlobals->g.PlayerRoles.statname[w] != OrigPlayerRoles.statname[w]) {                     \
      WCHAR buffer[256] = L"";                                                                     \
      PAL_swprintf(buffer, sizeof(buffer) / sizeof(WCHAR), L"%ls%ls%ls",                           \
                   PAL_GetWord(gpGlobals->g.PlayerRoles.rgwName[w]), PAL_GetWord(label),           \
      ...
      PAL_DrawNumber(gpGlobals->g.PlayerRoles.statname[w] - OrigPlayerRoles.statname[w], 5,        \
                     PAL_XY(183 + (maxNameWidth + maxPropertyWidth - 3) * 8, 74), kNumColorYellow, \
                     kNumAlignRight);                                                              \
      VIDEO_UpdateScreen(&rect);                                                                   \
      PAL_WaitForKey(3000);                                                                        \
    }                                                                                              \
...
#undef CHECK_HIDDEN_EXP
    ...
    while (j < gpGlobals->g.nLevelUpMagic) {
      ...
      if (PAL_AddMagic(w, gpGlobals->g.lprgLevelUpMagic[j].m[w].wMagic)) {
        ...
        PAL_DrawText(PAL_GetWord(gpGlobals->g.PlayerRoles.rgwName[w]), PAL_XY(75 - ww, 115), 0,
                     FALSE, FALSE, FALSE);
        PAL_DrawText(PAL_GetWord(BATTLEWIN_ADDMAGIC_LABEL), PAL_XY(75 + 16 * w1 - ww, 115), 0,
                     FALSE, FALSE, FALSE);
        PAL_DrawText(PAL_GetWord(gpGlobals->g.lprgLevelUpMagic[j].m[w].wMagic),
                     PAL_XY(75 + 16 * (w1 + w2) - ww, 115), 0x1B, FALSE, FALSE, FALSE);
  ...
  for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
    ...
    if (gpGlobals->g.PlayerRoles.rgwHP[w] == 0) {
      ...
      FLOAT f = (gpGlobals->g.rgLevelUpExp[gpGlobals->g.PlayerRoles.rgwLevel[w]] / 5.0f) /
                g_Battle.iExpGained;
          ...
          case 0:
            if (gpGlobals->wMaxPartyMemberIndex > 0) {
              g_Battle.rgPlayer[j].pos =
                  PAL_XY(PAL_X(g_Battle.rgPlayer[j].pos) + 4, PAL_Y(g_Battle.rgPlayer[j].pos) + 6);
              break;
            }

          case 1:
            g_Battle.rgPlayer[j].pos =
                PAL_XY(PAL_X(g_Battle.rgPlayer[j].pos) + 4, PAL_Y(g_Battle.rgPlayer[j].pos) + 4);
            break;

          case 2:
            g_Battle.rgPlayer[j].pos =
                PAL_XY(PAL_X(g_Battle.rgPlayer[j].pos) + 6, PAL_Y(g_Battle.rgPlayer[j].pos) + 3);
            break;
...
--*/
{
  ...
  for (i = 0; i < MAX_ENEMIES_IN_TEAM; i++) {
    ...
    if (w != 0) {
      ...
      } else if (g_Battle.rgEnemy[i].e.wExp == 250 && g_Battle.rgEnemy[i].e.wCash == 1100) {
      ...
      } else if (g_Battle.rgEnemy[i].e.wLevel < 20 && gpGlobals->wNumScene >= 0xD8 &&
                 gpGlobals->wNumScene <= 0xE2) {
        //
      ...
      } else if (g_Battle.rgEnemy[i].e.wLevel == 2 && g_Battle.rgEnemy[i].e.wCash == 48) {
        g_Battle.rgEnemy[i].e.wDexterity += 8;  // for Miao Fists
      } else if (g_Battle.rgEnemy[i].e.wLevel == 4 && g_Battle.rgEnemy[i].e.wCash == 240) {
        g_Battle.rgEnemy[i].e.wDexterity += 18;  // for Fat Miao
      } else if (g_Battle.rgEnemy[i].e.wLevel == 16 && g_Battle.rgEnemy[i].e.wMagicRate == 4 &&
  ...
  g_Battle.lpEffectSprite = (LPBYTE)UTIL_malloc(i);
