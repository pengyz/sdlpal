...
// Returns NULL on error, use SDL_GetError() to get more information.
SDL_STBIMG_DEF SDL_Texture *STBIMG_LoadTexture(SDL_Renderer *renderer, const char *file);

// loads the image file in the given memory buffer into a RGB(A) SDL_Texture
// Returns NULL on error, use SDL_GetError() to get more information.
SDL_STBIMG_DEF SDL_Texture *STBIMG_LoadTextureFromMemory(SDL_Renderer *renderer,
                                                         const unsigned char *buffer, int length);

// loads an image file into a RGB(A) SDL_Texture from a seekable SDL_RWops (src)
// if you set freesrc to non-zero, SDL_RWclose(src) will be executed after reading.
// Returns NULL on error, use SDL_GetError() to get more information.
SDL_STBIMG_DEF SDL_Texture *STBIMG_LoadTexture_RW(SDL_Renderer *renderer, SDL_RWops *src,
                                                  int freesrc);
...
// !  or byte-wise 32bit RGBA ("8888", bytesPerPixel=4) data !
// Returns NULL on error, use SDL_GetError() to get more information.
SDL_STBIMG_DEF SDL_Texture *STBIMG_CreateTexture(SDL_Renderer *renderer,
                                                 const unsigned char *pixelData, int width,
...
SDL_STBIMG_DEF SDL_Texture *STBIMG_LoadTexture_RW_noSeek(SDL_Renderer *renderer, SDL_RWops *src,
                                                         int freesrc);
#endif  // SDL_MAJOR_VERSION > 1
...
static SDL_Surface *STBIMG__CreateSurfaceImpl(STBIMG__image img, int freeWithSurface) {
  ...
  surf = SDL_CreateRGBSurfaceFrom((void *)img.data, img.w, img.h, img.format * 8,
                                  img.format * img.w, rmask, gmask, bmask, amask);
...
SDL_STBIMG_DEF SDL_Surface *STBIMG_LoadFromMemory(const unsigned char *buffer, int length) {
  ...
  if (ret == NULL) {
    // no need to log an error here, it was an SDL error which should still be available through
    // SDL_GetError()
    SDL_free(img.data);
    return NULL;
  }
...
static void STBIMG__io_skip(void *user, int n) {
  ...
  if (SDL_RWseek(io->src, n, RW_SEEK_CUR) == -1) {
    ...
    io->atEOF =
        2;  // set this to 2 for "aborting because seeking failed" (stb_image only cares about != 0)
  }
}
...
SDL_STBIMG_DEF SDL_Surface *STBIMG_Load_RW(SDL_RWops *src, int freesrc) {
  ...
  if (srcOffset < 0) {
    SDL_SetError(
        "STBIMG_Load_RW(): src must be seekable, maybe use STBIMG_Load_RW_noSeek() instead!");
  ...
  if (!inforet) {
    if (cbData.atEOF == 2)
      ...
      SDL_SetError("STBIMG_Load_RW(): Couldn't get image info: %s!\n", stbi_failure_reason());
    goto end;
  }
  ...
  img.data =
      stbi_load_from_callbacks(&cbData.stb_cbs, &cbData, &img.w, &img.h, &img.format, bppToUse);
  if (img.data == NULL) {
...
SDL_STBIMG_DEF SDL_Surface *STBIMG_CreateSurface(unsigned char *pixelData, int width, int height,
  ...
  if (bytesPerPixel != 3 && bytesPerPixel != 4) {
    SDL_SetError(
        "STBIMG_CreateSurface(): passed bytesPerPixel = %d, only 3 (24bit RGB) and 4 (32bit RGBA) "
        "are allowed!",
        bytesPerPixel);
    return NULL;
  }
...
SDL_STBIMG_DEF SDL_Texture *STBIMG_CreateTexture(SDL_Renderer *renderer,
                                                 ...
                                                 int height, int bytesPerPixel) {
  SDL_Surface *surf =
      STBIMG_CreateSurface((unsigned char *)pixelData, width, height, bytesPerPixel, SDL_FALSE);
