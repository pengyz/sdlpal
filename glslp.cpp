...
// uniform parameter is not needed since currently SDLPal lacks runtime configuation alter method
// like RetroArch GUI/kind of immediate UI/at least a quake console. lets consider it after anyone
// of them introduced:)
...
static int token_conform(const char *name, LineType type, GLSLP *pGLSLP) {
  ...
  switch (type) {
    case TOKEN_ORIGFILTER:
      if (SDL_strcasecmp(name, tokens[type]) == 0) index = 0;
      break;
    case TOKEN_SHADERS:
    case TOKEN_TEXTURES:
...
static char *strip_quotes(char *str) {
  ...
  while (begin[len] == '\r' || begin[len] == '\n' || begin[len] == '"') begin[len--] = '\0';
  return begin;
}

static BOOL line_tokenize(const char *line, int *sLength, LineType *pType, int *pIndex,
                          char *pValue, GLSLP *pGLSLP) {
  //
  // Skip leading spaces
  //
  ...
  if (*line && *line != '#') {
    ...
    if ((ptr = strchr(line, '=')) != NULL) {
      ...
      *end = '\0';
      ...
      while (isspace(value[len])) value[len--] = '\0';

      len = end - line;

      if (len > 0) {
        for (int i = (TOKEN_NONE + 1); i < TOKEN_END; i++) {
          int index = -1;
          if ((index = token_conform(name, (LineType)i, pGLSLP)) != -1) {
            *pType = (LineType)i;
            *pIndex = index;
...
static const char *scale_type_to_string(enum scale_type type) {
...
static const char *wrap_mode_to_string(enum wrap_mode type) {
  const char *value = NULL;
  switch (type) {
    case WRAP_REPEAT:
      value = "REPEAT";
...
static char *GLSLP_reflow(char *path) {
  ...
  while ((ptr = strstr(path, "/")) != NULL) *ptr = PAL_NATIVE_PATH_SEPARATOR[0];
  return path;
}
      ...
      if (line_tokenize(buf, &slen, &lineType, &index, value, pGLSLP)) {
        ...
        switch (lineType) {
          ...
          case TOKEN_SHADERS: {
            ...
            pGLSLP->shader_params =
                (shader_param *)UTIL_calloc(pGLSLP->shaders, sizeof(shader_param));
            for (int i = 0; i < pGLSLP->shaders; i++) {
          ...
          case TOKEN_SHADER_PATH:
            s_param->shader = strdup((UTIL_IsAbsolutePath(value) || strcmp(basedir, "./") == 0)
                                         ? value
                                         : PAL_va(0, "%s/%s", basedir, value));
            break;
          case TOKEN_SHADER_ALIAS:
          ...
          case TOKEN_TEXTURES: {
            ...
            if (pGLSLP->textures > 0) {
              pGLSLP->texture_params =
                  (texture_param *)UTIL_calloc(pGLSLP->textures, sizeof(texture_param));
              for (int i = 0; i < pGLSLP->textures; i++) {
                texture_param *param = &pGLSLP->texture_params[i];
          ...
          case TOKEN_TEXTURE_PATH:
            t_param->texture_path =
                strdup((UTIL_IsAbsolutePath(value) || strcmp(basedir, "./") == 0)
                           ? value
                           : PAL_va(0, "%s%s%s", basedir, PAL_NATIVE_PATH_SEPARATOR, value));
            break;
          case TOKEN_TEXTURE_WRAP_MODE:
          ...
          case TOKEN_PARAMETERS: {
            ...
            if (pGLSLP->uniform_parameters > 0) {
              pGLSLP->uniform_params =
                  (uniform_param *)UTIL_calloc(pGLSLP->uniform_parameters, sizeof(uniform_param));
              for (int i = 0; i < pGLSLP->uniform_parameters; i++) {
                uniform_param *param = &pGLSLP->uniform_params[i];
...
char *serialize_glslp(const GLSLP *pGLSLP) {
  char *output = (char *)UTIL_calloc(1, 65535);
  sprintf(output, "%s\r\n%s = %s", output, tokens[TOKEN_ORIGFILTER],
          pGLSLP->orig_filter ? pGLSLP->orig_filter : gConfig.pszShader);
  sprintf(output, "%s\r\n\r\n%s = %d", output, tokens[TOKEN_SHADERS], pGLSLP->shaders);
  for (int i = 0; i < pGLSLP->shaders; i++) {
    shader_param *param = &pGLSLP->shader_params[i];
    sprintf(output, "%s\r\n%s = %s", output, PAL_va(0, tokens[TOKEN_SHADER_PATH], i),
            param->shader);
    sprintf(output, "%s\r\n%s = %s", output, PAL_va(0, tokens[TOKEN_SHADER_ALIAS], i),
            param->alias ? param->alias : "");
    sprintf(output, "%s\r\n%s = %s", output, PAL_va(0, tokens[TOKEN_SHADER_FILTER_LINEAR], i),
            param->filter_linear ? "true" : "false");
    sprintf(output, "%s\r\n%s = %s", output, PAL_va(0, tokens[TOKEN_SHADER_WRAR_MODE], i),
            wrap_mode_to_string(param->wrap_mode));
    sprintf(output, "%s\r\n%s = %s", output, PAL_va(0, tokens[TOKEN_SHADER_SCALE_TYPE_X], i),
            scale_type_to_string(param->scale_type_x));
    sprintf(output, "%s\r\n%s = %s", output, PAL_va(0, tokens[TOKEN_SHADER_SCALE_TYPE_Y], i),
            scale_type_to_string(param->scale_type_y));
    sprintf(output, "%s\r\n%s = %.1f", output, PAL_va(0, tokens[TOKEN_SHADER_SCALE_X], i),
            param->scale_x);
    sprintf(output, "%s\r\n%s = %.1f", output, PAL_va(0, tokens[TOKEN_SHADER_SCALE_Y], i),
            param->scale_y);
    sprintf(output, "%s\r\n%s = %s", output, PAL_va(0, tokens[TOKEN_SHADER_FLOAT_FRAMEBUFFER], i),
            param->float_framebuffer ? "true" : "false");
    sprintf(output, "%s\r\n%s = %s", output, PAL_va(0, tokens[TOKEN_SHADER_SRGB_FRAMEBUFFER], i),
            param->srgb_framebuffer ? "true" : "false");
    sprintf(output, "%s\r\n%s = %s", output, PAL_va(0, tokens[TOKEN_SHADER_MIPMAP_INPUT], i),
            param->mipmap_input ? "true" : "false");
    sprintf(output, "%s\r\n%s = %d", output, PAL_va(0, tokens[TOKEN_SHADER_FRAME_COUNT_MOD], i),
            param->frame_count_mod);
  }
  sprintf(output, "%s\r\n\r\n%s = \"%s\"", output, tokens[TOKEN_TEXTURES],
          glslp_pack_texturenames(pGLSLP));
  for (int i = 0; i < pGLSLP->textures; i++) {
    texture_param *param = &pGLSLP->texture_params[i];
    sprintf(output, "%s\r\n%s = %s", output,
            PAL_va(0, tokens[TOKEN_TEXTURE_PATH], param->texture_name), param->texture_path);
    sprintf(output, "%s\r\n%s = %s", output,
            PAL_va(0, tokens[TOKEN_TEXTURE_LINEAR], param->texture_name),
            param->linear ? "true" : "false");
    sprintf(output, "%s\r\n%s = %s", output,
            PAL_va(0, tokens[TOKEN_TEXTURE_MIPMAP], param->texture_name),
            param->mipmap ? "true" : "false");
    sprintf(output, "%s\r\n%s = %s", output,
            PAL_va(0, tokens[TOKEN_TEXTURE_WRAP_MODE], param->texture_name),
            wrap_mode_to_string(param->wrap_mode));
  }
  sprintf(output, "%s\r\n\r\n%s = \"%s\"", output, tokens[TOKEN_PARAMETERS],
          glslp_pack_parameters(pGLSLP));
  for (int i = 0; i < pGLSLP->uniform_parameters; i++) {
    uniform_param *param = &pGLSLP->uniform_params[i];
...
void glslp_add_parameter(char *line, size_t len, GLSLP *pGLSLP) {
  ...
  tempParam.parameter_name = (char *)UTIL_calloc(1, PAL_MAX_PATH);
  tempParam.desc = (char *)UTIL_calloc(1, PAL_MAX_PATH);
  int found = -1, nfound = 0;
  sscanf(line, "#pragma parameter %63s \"%63[^\"]\" %lf %lf %lf %lf", tempParam.parameter_name,
         tempParam.desc, &tempParam.value_default, &tempParam.minimum, &tempParam.maximum,
         &tempParam.step);
  UTIL_LogOutput(LOGLEVEL_INFO, "Found #pragma parameter %s (%s) %.6f %.6f %.6f %.6f\n",
                 tempParam.desc, tempParam.parameter_name, tempParam.value_default,
                 tempParam.minimum, tempParam.maximum, tempParam.step);
  for (int i = 0; i < pGLSLP->uniform_parameters; i++) {
    uniform_param *param = &pGLSLP->uniform_params[i];
    if (param->parameter_name && strncmp(param->parameter_name, tempParam.parameter_name,
                                         strlen(tempParam.parameter_name)) == 0) {
      found = i;
    }
  }
  if (found == -1) {
    ...
    pGLSLP->uniform_params = (uniform_param *)realloc(
        pGLSLP->uniform_params, pGLSLP->uniform_parameters * sizeof(uniform_param));
  }
...
void destroy_glslp(GLSLP *pGLSLP) {
  if (pGLSLP->orig_filter) free(pGLSLP->orig_filter);
  for (int i = 0; i < pGLSLP->shaders; i++) {
    shader_param *param = &pGLSLP->shader_params[i];
    if (param->shader) free(param->shader);
    if (param->alias) free(param->alias);
    if (param->pass_sdl_texture) SDL_DestroyTexture(pGLSLP->shader_params[i].pass_sdl_texture);
  }
  free(pGLSLP->shader_params);
  for (int i = 0; i < pGLSLP->textures; i++) {
    texture_param *param = &pGLSLP->texture_params[i];
    if (param->texture_name) free(param->texture_name);
    if (param->texture_path) free(param->texture_path);
    if (param->sdl_texture) SDL_DestroyTexture(pGLSLP->texture_params[i].sdl_texture);
  }
  free(pGLSLP->texture_params);
  for (int i = 0; i < pGLSLP->uniform_parameters; i++) {
    uniform_param *param = &pGLSLP->uniform_params[i];
    if (param->parameter_name) free(param->parameter_name);
    if (param->desc) free(param->desc);
  }
